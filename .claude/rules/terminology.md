...
### 文書参照・表記の精度（R-2 T9 / T12 / T13 由来 / M-1 W1 判定 = 圧縮形で実施）

- **文書内の相互参照は § 見出し表記を用いる**（hardcoded な `L\d+` 行番号参照を使わない / 行番号は挿入でずれる）
- **表・節番号の挿入は末尾追加を既定**とし、途中挿入が必要なら枝番（`§4.5`）を用いる。全体 re-numbering は参照元をすべて追随できる場合のみ
- **成果物ファイル名の日付**: レポート・分析文書は**起草日**、ログ・JSON 等の実行成果物は**実行日**
