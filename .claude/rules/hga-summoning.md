...
> **移行期規定（MUST / ADR-0009 追補）**: **M-1（`docs/specs/m-1-opus5-migration/`）の実施中は下記「旧ゲート」を適用する。「新ゲート」は M-1 完了（W4 retro）後に発効する。**
> 理由: M-1 自体が「spec/design 初期の不可逆な設計コミット」であり、新ゲートを M-1 に適用すると**新ゲートの妥当性検証を新ゲート自身に委ねる**構造になるため。

### 新ゲート（M-1 完了後に発効 / ADR-0011 決定 4）
...
以下いずれかに該当する場合に Fable を召喚する（内部確信度に依存しない、事前観測可能な軸）。**新ゲート発効後、上 3 条件のうち条件 2 の判定材料へ格下げされる**（spec/design 初期・不可逆な設計コミット・新規／複数ドメイン統合の 3 軸は「無条件召喚」の資格を失う）。
