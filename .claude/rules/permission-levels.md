...
- ルート `CLAUDE.md` の変更（無条件ロードされる最も常駐性の高い 1 ファイル / 2026-07-26 追加）
- 公開 API の変更
...
| ルート `CLAUDE.md` | PM | 憲法変更（**nested な `CLAUDE.md` は含まない** —— `/compact` 後に再注入されるのがプロジェクトルートのみで、常駐性の根拠がルートに限られるため）|
