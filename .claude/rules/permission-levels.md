...
> **既知の乖離（未入場 / 2026-08-26 時点）**: 実装 `_hook_utils.py` `_PM_PATH_PATTERNS` は **ルート `CLAUDE.md`** も PM 判定する（2026-07-26 追加）。**本表には意図的に載せていない** —— 条文への追加は誕生ゲート上の入場に当たり、**真正な退出 2 件**を要するため（`docs/artifacts/clause-gate-ledger.md` §D 在庫 #5 / 2026-08-26 に無料で閉じる試みが gabriel probe に差し戻された）。したがって **hook は `CLAUDE.md` 編集に PM ダイアログを出すが、条文上 L1 は事前宣言義務を負わない**。この状態は既知であり、沈黙した乖離ではない。
>
