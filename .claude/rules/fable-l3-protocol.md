...
宣言直前に走らせる。1 つでも No が残れば宣言禁止 (△印は明記可)。全項目は原典を正本とし、以下は要点のみ:

**A. 要求照合** (1-3): 依頼文の**いま**読み直し / 明示要求に対応物 / 目的に役立つ状態

**B. 動作事実** (4-7): 実行 or テスト 1 回 / 報告「動く」は事実 (△) / 新異常確認 / 壊しうる既存 1 つ挙げて確認

**C. 波及** (8-10): 同型パターン検索 (△: 検索と 1 件目修正必須) / 嘘文書更新 (△) / 数値検算

**D. 受け手体験** (11-12): **最初の 60 秒時系列実況** (省略・要約不可 / 実況最初の 1 文を成果物監査欄に残す / §5 と同一の実況で二重生成不要) / 「明日困ること」(△)

**E. 報告** (13-14): 結論先出し・失敗正直・デフォルト理由 / 次の一手明確
...
subagent 完了報告時の実況は**不要**。subagent はユーザー文脈 (過去の好み・セッション経緯) を持たず実況の解像度が構造的に低い。L1 が宣言境界で 1 回実施すれば足りる (二重生成禁止と整合)。
...
- **ガード 1 (主語チェック)**: 実況第 1 文の主語が「受け手」でない (例: 「このコミットは〜」で始まる) 場合、再実況要求
- **ガード 2 (同型構文検出)**: 過去 3 回分の実況第 1 文を並置し構文が同型なら形骸化として原典 §4-1 書式規則 (「詰まり仮説 1 行を先に立てる」) を再注入 + retro 議題化。**発火点数の一時的減少は禁止** (検知器自体が減る)
- **ガード 3 (「詰まる瞬間」検出)**: 実況内に「詰まる / 迷う / 探す / 戻る / なぜ / どこ」等の疑問・不安ワードがゼロなら形骸化の兆候。ただし単発ゼロで再実況を強制すると違和感のでっち上げが起きる (原典 §4 後始末規則「違和感ゼロは正常」に抵触) ため、**3 回連続ゼロで閾値発火**
...
- **ガード 6 (検知修正 1 文完結禁止)**: 違和感検出時は「何をどう直したか」1 行を残す。「〜で迷うかもしれないが表に書いてあるので大丈夫」で修正実体をスキップする形骸化を禁ずる

意味判定 (同型構文・詰まる瞬間の有無) は hook で機構化不能 (正規表現では精度が出ない)。ガード 2/3 は `/retro` バッチで Haiku 判定 (事実突合の既存運用圏内)。第 1 文の存在チェックのみ hook で機構化可能。
...
`/auditing` フェーズ開始時 **1 箇所に固定** (HGA #12 Q4 (n) 判定 / `/ship` Phase 5 後発火は禁止)。

`phase-rules.md` AUDITING §コード品質チェック直後にチェックリスト形式で配置:

1. 作った層と別の層で確認する (コード → 実行 / 文章 → 初見の読者として読む = 60 秒実況のコード版)
2. 壊しに行く (空入力・境界値・変な順に当てる)
3. 原因の説明を観測で裏取りする
4. 報告に書く各原因に「症状への寄与の証拠」を 1 つずつ持て。なければ「寄与は未確認」と書け
5. 「コードが直っていても説明が観測と矛盾していればその報告は嘘だ」
