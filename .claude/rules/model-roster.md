...
### 実行モードの担当層（W2-M1-T3 で `CLAUDE.md` §Execution Modes から SSOT 退避）

| モード | 用途 | 担当層 | 現行モデル |
|:-------|:-----|:-------|:-----------|
| `/building` | TDD 実装（仕様確認必須） | **L2** | Sonnet 5 |

モードの規律本体（ガードレール）は `.claude/rules/phase-rules.md` / `CLAUDE.md` §Execution Modes。本表が持つのは**担当層 → モデルの束縛のみ**。
...
| 2 | **L1 直作業の自己チェック** | Edit 5 回 + Write 1 回を超えるなら「L2 に委譲できないか」を自問する | `CLAUDE.md` §担当層の判断基準 |
...
## §5 モデル運用（試験運用中の世代 / W2-M1-T3 で `CLAUDE.md` から移設）

新世代を L1 に採用した直後は「試験運用」として扱い、以下を適用する。

| 項目 | 現行（2026-07-26 時点） |
|:-----|:------------------------|
| 試験運用中の世代 | **Opus 5**（2026-07-25 採用 / **安定性ゲート合格済** 2026-07-26） |
| フォールバック先 | **Opus 4.7（1M）** |
| 切替条件 | malformed / tool 呼び出し異常が発生した場合は即フォールバックし、インシデントを `docs/artifacts/` に起票する |
| 超大型コンテキスト投入時 | 1M を維持できる世代を選ぶ（現行 Opus 5 は 1M が default かつ最大） |

- ゲート判定の記録: `docs/artifacts/m-1-opus5-stability-gate.md`
- 過去の malformed 追跡: `docs/artifacts/incident-2026-06-02-tool-malformed.md` §追跡調査（**Opus 4.x 世代の観測**であり、Opus 5 への据え置き根拠としては失効している）
...
> **手順 2 と 3 は実装済**（2026-08-20 確認 / `verify_model_reference.py` は `7454629` 時点で稼働 / `/update-model` も実在）。旧記述はこの 2 つを「未実装」として目視確認へ誘導していたが、**存在する検査を使わせない誤った処方**だったため撤去した。
