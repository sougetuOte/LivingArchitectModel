...
**BUILDING** (**Milestone なし** / D-1 は 2026-08-13 クローズ / 配置是正・診断を Milestone 外で継続)

_2026-08-20 是正: 本ファイルの注釈が **2026-07-27（セッション 17 末）のまま stale** だった。冒頭が「Milestone D-1 / Wave 1 の T1 から着手」を指していたが、**D-1（配布境界の決定）は 2026-08-13（セッション 19）に完走・クローズ済**（判定 P=9 / X=1 / S=5 / retro = `docs/artifacts/retro-D1-2026-08-13.md` / **v5.0.1 リリース**）であり、**以後 Milestone は持たない**。**フェーズ値 `BUILDING` 自体は正しい** —— 直近（セッション 21 / 2026-08-17）の作業は **R3 機構 #7（曝露検出）の TDD 実装**と **`rule-001` 恒久解 (c)（パーサの宣言欄正本化）**であり、実装コード生成を伴うため BUILDING が適切な値である。現在の作業は Milestone 外の配置是正・診断（次の 3 件 = T5 の検査 / HGA 頻度を上げる試行の判定 / retro-D1 残余の配布判定）。**本ファイルの stale が実害を持つことは 2026-07-27 の是正で既に実証済**（`pre-tool-use.py` が本ファイルを読んでおり、下記 2026-07-27 更新の段を参照）。_
...
| BUILDING | 2026-07-27 | 2026-07-27 | **HGA #24 の手 2 を実装**（`phase-rules.md` PLANNING §禁止 4 項の hook 化 + Outbound Write Ban の R3 二重化の判定）。**唯一の既知 enforcement ギャップ**を閉じる配置是正であり、予算外・Milestone なし |
| PLANNING | 2026-07-27 | 2026-07-27 | **Milestone D-1 を起票**（配布境界の決定 / 根拠 = `docs/artifacts/d-1-rationale-2026-07-27.md`）。requirements v2.0 / design v0.2 / tasks v0.1 の 3 成果物すべて承認 |
| **BUILDING** | **2026-07-27** | **2026-08-13** | **D-1 完走・クローズ**（W1-D1-T1 証拠表 → T2 判定 = **P=9 / X=1 / S=5** / `docs/artifacts/d-1-evidence-2026-07-28.md` §6）。retro = `docs/artifacts/retro-D1-2026-08-13.md` / 200K 閾値条項撤廃 / **v5.0.1 リリース**。**以後 Milestone なし**（配置是正・診断を Milestone 外で継続 / 2026-08-17 = R3 機構 #7 + `rule-001` 恒久解） |
