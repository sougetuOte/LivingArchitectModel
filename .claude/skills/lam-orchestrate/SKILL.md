...
2. 状態更新（各イテレーション完了時）
   ...
   → issues_found, issues_fixed, pg/se/pm 件数
...
| hook | 参照タイミング | 動作 |
|------|-------------|------|
| **Stop hook** (`lam-stop-hook.sh`) | Claude 応答完了時 | `active: true` なら Green State 判定。未達なら `block` で継続 |
| **PostToolUse hook** (`post-tool-use.sh`) | ツール実行後 | `active: true` ならツール実行結果を `log[]` に追記 |
| **PreToolUse hook** (`pre-tool-use.sh`) | ツール実行前 | ループ中も PG/SE/PM 権限判定は通常通り適用 |
...
lam-orchestrate は `/full-review` の Phase 0 で呼び出され、以下を実行する:

1. **Phase 0（初期化）**: 状態ファイルを生成し、ループパラメータを設定
2. **Phase 1-4（監査ループ）**: `/full-review` が Phase 1-4 を実行。lam-orchestrate は状態管理のみ担当
3. **Phase 5（終了）**: lam-orchestrate が状態ファイルを削除し、ループログを保存
