...
| hook | 参照タイミング | 動作 |
|------|-------------|------|
| **Stop hook** (`lam-stop-hook.py`) | Claude 応答完了時 | `active: true` なら安全ネットとして `block`（Green State 判定は Claude 側の責務。`lam-loop-state.json` の削除で停止許可） |
...
Stage 5 の差分チェックで Green State を達成した場合、フルスキャンを発動するためにフラグをセットする:
