...
## 構造化思考（AoT + Three Agents）

複雑な判断が必要な場面で、AoT + Three Agents + Reflection を統合した構造化思考を発動する。
...
```
Phase 0: Grounding + 複雑度判定 → Level 1: AoT → Level 2: Debate → Level 3: Reflection → Synthesis
```
...
思考過程を `docs/artifacts/YYYY-MM-DD-lam-think-{用途}.md` に書き出す（可視化・追跡可能）。
- 書き込み権限: Mediator のみ（Single-Writer）
- 読み取り権限: 全 Agent（Multi-Reader）
- 削除: ユーザーのみ可能
- フォーマット: `.claude/skills/lam-orchestrate/references/anchor-format.md` を参照

### 参照

- `docs/internal/06_DECISION_MAKING.md` — AoT + Three Agents の SSOT
...
**スキーマ定義（SSOT）**: `/full-review` コマンド（`.claude/commands/full-review.md` Phase 0）を参照。
