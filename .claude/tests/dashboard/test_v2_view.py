...
def test_render_v2_contains_table(make_milestone):
...
def test_render_v2_contains_tbody(make_milestone):
...
def test_render_v2_milestone_name_in_row(make_milestone):
...
def test_render_v2_anchor_link_format(make_milestone):
...
def test_render_v2_anchor_link_for_different_milestone(make_milestone):
    """B-4 など別 Milestone でも data-milestone 属性と h3 タイトルが正しく生成されること。"""
    milestone = make_milestone(name="B-4")
    html = _make_builder(milestones=[milestone]).render()
...
def test_render_v2_multiple_milestones(make_milestone):
...
def test_render_v2_step_column_shows_current_phase(make_milestone):
...
def test_render_v2_row_has_data_milestone_attribute(make_milestone):
...
def test_v1_section_not_broken_by_v2(make_milestone):
...
def test_render_v2_contains_h2_heading(make_milestone):
