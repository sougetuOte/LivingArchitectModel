...
def test_v3_section_id_contains_milestone_name(make_wave):
...
def test_v3_section_per_milestone_b4(make_wave):
    """B-4 Milestone の Wave も <section id="v3-waves-B-4"> で生成されること。"""
    wave = make_wave(milestone="B-4")
    html = _make_builder(waves=[wave]).render()
...
def test_v3_separate_sections_for_different_milestones(make_wave):
    """B-4 と B-5 の Wave がある場合、両方のセクションが生成されること。"""
    wave_b4 = make_wave(milestone="B-4", wave_number="7")
    wave_b5 = make_wave(milestone="B-5", wave_number="1")
...
def test_v3_contains_table(make_wave):
...
def test_v3_thead_has_three_columns(make_wave):
...
def test_v3_contains_tbody(make_wave):
...
def test_v3_wave_number_displayed(make_wave):
    """Wave 番号「Wave 1」が行に表示されること。

    仕様: design.md §4 V-3 DOM 構成案「<td>Wave 1</td>」
    """
    wave = make_wave(wave_number="1")
    html = _make_builder(waves=[wave]).render()
    assert "Wave 1" in html, "「Wave 1」が HTML に見つかりません。"


def test_v3_wave_number_15_displayed(make_wave):
    """Wave 番号「Wave 1.5」が行に表示されること（小数点形式）。"""
    wave = make_wave(wave_number="1.5")
    html = _make_builder(waves=[wave]).render()
    assert "Wave 1.5" in html, "「Wave 1.5」が HTML に見つかりません。"


def test_v3_wave_number_3_displayed(make_wave):
    """Wave 番号「Wave 3」が行に表示されること。"""
    wave = make_wave(wave_number="3")
    html = _make_builder(waves=[wave]).render()
...
def test_v3_task_count_displayed(make_wave):
    """Task 数が行に表示されること。

    仕様: design.md §4 V-3 DOM 構成案「<td>5</td>」（task_count の値）
    """
    wave = make_wave(task_count=5)
    html = _make_builder(waves=[wave]).render()
    assert ">5<" in html, "Task 数「5」が HTML に見つかりません。"


def test_v3_task_count_zero_displayed(make_wave):
    """Task 数 0 が行に表示されること（境界値テスト）。"""
    wave = make_wave(task_count=0)
    html = _make_builder(waves=[wave]).render()
    assert ">0<" in html, "Task 数「0」が HTML に見つかりません。"


def test_v3_task_count_large_displayed(make_wave):
    """Task 数が大きい場合も正しく表示されること。"""
    wave = make_wave(task_count=12)
    html = _make_builder(waves=[wave]).render()
...
def test_v3_anchor_link_to_v4_tasks(make_wave):
...
def test_v3_row_has_data_wave_attribute(make_wave):
...
def test_v3_row_data_wave_attribute_15(make_wave):
...
def test_v3_section_has_h2_heading(make_wave):
...
def test_v3_h2_heading_contains_wave_list_label(make_wave):
...
def test_v3_multiple_waves_same_milestone(make_wave):
    """同じ Milestone の複数 Wave が全て表示されること。"""
    wave1 = make_wave(milestone="B-5", wave_number="1", status="completed")
    wave2 = make_wave(milestone="B-5", wave_number="2", status="in-progress")
    wave3 = make_wave(milestone="B-5", wave_number="3", status="not-started")
...
def test_v3_waves_grouped_by_milestone(make_wave):
...
def test_v3_milestone_name_html_escaped(make_wave):
...
def test_v1_section_not_broken_by_v3(make_wave):
...
def test_v2_section_not_broken_by_v3(make_milestone, make_wave):
    """V-3 実装後も V-2 セクションが正常に生成されること（回帰テスト）。"""
    ms = make_milestone(name="B-5")
    wave = make_wave()
    from dashboard.builder import DashboardBuilder
    from dashboard.models import DashboardData
...
def test_v3_position_before_parser_errors(make_wave):
