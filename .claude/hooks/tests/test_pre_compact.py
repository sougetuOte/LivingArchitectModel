...
テスト戦略: hooks 側 conftest.py の project_root（tmp）を用い、pre_compact fixture で
           モジュールを import して各関数を個別検証する（in-process）。

移植元: tests/test_pre_compact.py（判断2 テスト一元化・2026-06-02）。関数単位の
ユニットテストのため subprocess(hook_runner) ではなく in-process を維持する。
"""
...
# テスト対象モジュールのディレクトリ（.claude/hooks/）
