...
def _save_loop_log(project_root: Path, state: dict, log_file: Path) -> None:
    ...
    try:
        ...
        loop_log_file = logs_dir / f"loop-{datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%d-%H%M%S')}.txt"
        lines = [
            "=== Loop Log ===",
            f"timestamp: {now}",
...
def _detect_security_tools(check_dir: Path) -> list[tuple[str, list[str]]]:
    ...
    elif (check_dir / "pyproject.toml").exists() or (check_dir / "requirements.txt").exists():
...
def _run_tests(check_dir: Path, log_file: Path) -> tuple[int, int]:
    ...
    result: RESULT_UNKNOWN / RESULT_PASS / RESULT_FAIL
...
def _run_security(check_dir: Path, log_file: Path) -> int:
    ...
    if src_dir.is_dir():
        ...
        safe_pattern = re.compile(
            r"(test|spec|mock|example|placeholder|TODO|FIXME|xxx|changeme)",
            re.IGNORECASE,
        )
        secret_count = 0
        for src_file in src_dir.rglob("*"):
            ...
            try:
                ...
                for line in content.splitlines():
                    if secret_pattern.search(line) and not safe_pattern.search(line):
                        secret_count += 1
            except Exception:
                pass
        if secret_count > 0:
...
def main() -> None:
    ...
    if pre_compact_flag.exists():
        try:
            ...
            if elapsed <= PRE_COMPACT_THRESHOLD_SECONDS:
                _log(log_file, "WARN", f"PreCompact fired {elapsed:.0f}s ago → context pressure, stop loop")
                try:
                    state_file.unlink()
                except Exception:
                    pass
            ...
            try:
                ...
                if elapsed <= PRE_COMPACT_THRESHOLD_SECONDS:
                    _log(log_file, "WARN", f"PreCompact fired {elapsed:.0f}s ago (mtime) → context pressure, stop loop")
                    try:
                        state_file.unlink()
                    except Exception:
                        pass
    ...
    if _check_issue_recurrence(state):
        _log(log_file, "WARN", "ESC: same issues recurring (no fix for 2 cycles) → escalate to human")
        try:
            state_file.unlink()
        except Exception:
            pass
    ...
    if test_count > 0 and log_entries:
        try:
            state_refreshed = json.loads(state_file.read_text(encoding="utf-8"))
            if state_refreshed.get("log"):
                state_refreshed["log"][-1]["test_count"] = test_count
                atomic_write_json(state_file, state_refreshed)
        except Exception:
            pass
