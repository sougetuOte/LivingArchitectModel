...
### 2.2 AUTONOMOUS モードでの適用範囲

AUTONOMOUS モード（`docs/specs/autonomous-mode/`）の完了判定は、
**Wave 2 まで G1（テスト全パス）のみ** を使用する。
G2 / G5 の組み込みは autonomous-mode の T2-1 で追加予定であり、
それまでは本仕様の「MVP ○」マークと AUTONOMOUS の完了判定条件は一致しない（意図的な段階導入）。
