...
- `max_iterations`（暫定 20）: 無限ループ防止の上限。到達で `active=false` 停止。外部化し調整可。
  - ⚠️ **Stop hook block cap 制約（T0-1 裏取り）**: Claude Code は Stop hook が **8回連続 block すると override してターン終了**する。`max_iterations`（20）を活かすには、自律ラン中は環境変数 **`CLAUDE_CODE_STOP_HOOK_BLOCK_CAP`** を `max_iterations` 以上に引き上げ、かつ `lam-stop-hook` は入力の **`stop_hook_active`** を見て進捗がなければ早期 exit して停止を許す（無限 block 回避）。
...
**MVP（Wave 1）の採用**: 層1（`permissions.deny`）に FR-9 統治ファイル（`.claude/rules/**` / `docs/adr/**` / `.claude/settings*.json` / `.claude/hooks/**` / `.claude/skills/autonomous/**`）を**上書き不可**で固定する。これは D5 の PreToolUse hook 強制と**二重防御**（hook はプロンプティング層、permissions.deny は決定的層）。
...
> 注: auto mode は Anthropic API のみ（Bedrock/Vertex/Foundry 不可）。`autoMode` 設定は共有 `.claude/settings.json` からは allow を注入できず、managed/user/local settings 経由（findings §C）。
