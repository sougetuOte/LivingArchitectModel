...
最終更新: 2026-06-02 / フェーズ: PLANNING（design）/ ステータス: **approved（2026-05-30 ユーザー承認）**
...
- **不可逆 C はキュー化しない（FR-3.4）**: `security-commands.md` の deny 系（rm / push / 秘密露出 / spec・ADR・rules・hooks 書換）は `pm_queue` を経由せず**即時ハードストップ**。ADR/rules/hooks は D5 の FR-9 強制（統治ファイルの不可侵）で、**spec は FR-9 の統治ファイルではなく成果物だが FR-3.4 固有の spec freeze として**、いずれも PreToolUse の deny（層2 `_FR9_PATTERNS` / `_FR34_SPEC_PATTERNS`）+ `permissions.deny`（層1・D7）で実現する。
...
**FR-3.4 spec freeze（FR-9 とは別系統の即時ハードストップ）**:

spec（`docs/specs/**`）は FR-9 の統治ファイル（自己統治の強制点）ではなく成果物だが、FR-3.4（MUST）が「spec 書換」を不可逆 C 操作の即時ハードストップに明示列挙する（requirements FR-3.4）。よって AUTONOMOUS では spec も書込 deny する。FR-9 と混同しないよう、層2 は別パターン群（`_FR34_SPEC_PATTERNS`）、層1 は `permissions.deny` に `Edit/Write(/docs/specs/**)` を併記する（二重防御）。

- **autonomous は spec-frozen**: BUILDING の仕様同期ルール S-4（Refactor で公開 API/インターフェースが変わった場合に spec を即時更新）は autonomous では発火させない。API 変更級の spec 変更は FR-3.4 の PM ハードストップとして人間承認ゲートに戻す（自律ループ内で spec を書き換えない）。
...
| `.claude/hooks/pre-tool-use.py` | FR-9 強制 + FR-3.4 spec freeze + merge前ゲート条件分岐 | SE |
| `.claude/hooks/checkers/`（新規） | G1/G2/G5 を実行する checker スクリプト | SE |
| `.claude/settings.json`（`permissions.deny` + `autoMode`） | FR-9 統治ファイル + FR-3.4 spec を上書き不可で deny（D7 層1）+ auto mode 設定 | **PM**（settings 変更） |
...
| 2026-06-02 | Living Architect(Opus) + sougetuOte | 監査 CP-C 判断1（仕様準拠修正）: FR-3.4 が列挙する「spec 書換」の即時ハードストップに強制点が存在しない設計内部矛盾を解消。D2 §4 の spec 強制経路を正確化（D5 の FR-9 ではなく FR-3.4 固有の spec freeze）+ D5 に spec freeze 小節を追加（BUILDING S-4 との衝突を明文解消）。実装は層1（`settings.autonomous.json` に `docs/specs/**` deny）+ 層2（`pre-tool-use.py` に `_FR34_SPEC_PATTERNS`） |
