...
**`/goal` completion 接続**: 自律ループの完了条件（`done`）は `/goal` の検証可能な完了条件として宣言する。`/goal` の validator は毎ステップ「Green State 達成か」を問い、達成まで自律継続する（FR-1.1 / FR-7.1 の resumable と整合）。
...
| SC-7（統治ファイル無断改変なし）| D5 の FR-9 PreToolUse hook 強制（autonomous 自身も対象） |
