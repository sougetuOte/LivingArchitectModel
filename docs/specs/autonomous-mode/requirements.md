...
- **FR-1.3 (SHOULD)**: 既存の PLANNING/BUILDING/AUDITING 三フェーズ規律と整合する形で追加されるべきである（RQ-6 により独立第4モードに確定。§6 参照）。
...
- **FR-4.1a (MUST)**: 完了判定は決定的 Green State シグナル（G1 テスト exit0 / G2 lint exit0 / G5 セキュリティ）に接地しなければならない。
- **FR-4.1b (MUST NOT)**: 「モデルが達成と宣言したこと」を完了の唯一根拠にしてはならない。
...
- **FR-9.1 (MUST NOT)**: 自律エンジン（/goal ループおよび将来の DW 実行を含む）は、自身の統治を定義するファイル群 —— `.claude/rules/` / `docs/adr/` / `.claude/settings*.json` / `.claude/hooks/`（PreToolUse の deny/PM 判定等、統治の強制点） / 自律モード自身のスキル・スクリプト定義 —— への書込権限を持ってはならない。これらの変更は常に自律ループの*外側*の人間承認ゲート（PM）を経由しなければならない（自己破壊的再帰の防止）。
  - **FR-9.1 補足（DENY 範囲の意図）**: `.claude/hooks/` の書込禁止は `hooks/tests/`・`hooks/checkers/` を含む**ツリー全体に意図的に適用する**。テスト・checker の書換は Green State 判定（G1/G5）の偽装経路となるため、自律ループ内でのテスト修正イテレーションは許可しない（tests/checkers の除外パターン化は不採用）。AUTONOMOUS 中のみ deny→ask へ緩和する代替案は将来検討の余地ありとして [future-candidates.md](./future-candidates.md) に記録する（2026-06-10 B-2 監査 PM-1 で確定）。
...
- **FR-9.3 (SHOULD)**: 機械的検証の合意収束（adversarial verify 等）を決定的シグナルと見なしてはならない。相関した盲点で偽収束しうるため、完了判定は FR-4.1a の決定的 Green State 接地を唯一の根拠とし、合意収束は非決定的観点（G3/G4）の補助に留めるべきである。
...
- **SC-4**: 自律モード導入直前のテストスイート（参考値: 2026-05 時点 576 passed / 20 skipped）が回帰なく全通過する。
...
- **SC-7**: 自律モードのいかなる構成でも、自律エンジンが自身の統治ファイル（`.claude/rules/` / `docs/adr/` / `.claude/settings*.json` / `.claude/hooks/` / モード自身の定義）を無断改変せず、worktree 隔離を C層遮断と混同しないこと（設計・ログで検証可能）。
...
| FR-3 | ユーザー選好（バッチ）+ ADR-0005 CASPAR 方針6（キュー bounded+merge前ゲート）| キュー + merge ゲート |
...
| 2026-05-30 | sougetuOte+LA | レビュー反映: FR-9.1/SC-7 に hook 明示・FR-1.3 確定反映・FR-4.1 を a/b 分割・SC-4 計測可能化・§7 CASPAR 表記明示 |
| 2026-06-10 | sougetuOte+LA | B-2 監査 PM-1: FR-9.1 補足追加（hooks/tests/ 含む全体 DENY は意図的・除外パターン化不採用・ask 緩和案は future-candidates へ） |
