...
| 対象 | 目安 | 実装ハードタイムアウト |
|------|------|---------------------|
| `gitleaks protect --staged`（差分） | 5秒以内 | 60秒 |
| `gitleaks detect`（全体、~30K行） | 30秒以内 | 120秒 |

「目安」列は ~30K 行規模での **性能目標値**。実装のハードタイムアウトを
目標値より大きく取るのは意図的であり、大規模リポジトリでの誤タイムアウトにより
スキャンが打ち切られてシークレット検出が偽陰性化することを防ぐため。
タイムアウト発生時は `gitleaks:scan-timeout` の Critical Issue として顕在化する（黙殺しない）。
