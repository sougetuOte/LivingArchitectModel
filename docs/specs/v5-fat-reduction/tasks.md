...
| **関連ファイル** | `.claude/hooks/tests/test_distill_lessons.py`、`.claude/scripts/distill_lessons.py` |
...
- [ ] 実装前に `pytest .claude/hooks/tests/test_distill_lessons.py -v` を実行し、既存 21 件が PASS していることを確認する（ベースライン記録）
- [ ] 実装後に `pytest .claude/hooks/tests/` で FAIL=0 を確認する（NFR-V2）
...
| B | `.claude/scripts/distill_lessons.py`、`.claude/hooks/tests/test_distill_lessons.py` | なし |
