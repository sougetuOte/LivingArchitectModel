...
| `distill()` に C-1〜C-4 全条件の空スキップが実装されている | `pytest .claude/scripts/tests/test_distill_lessons.py` FAIL=0 | AC-6 |
| 既存テスト 21 件 + 新規 3 件が全 PASS | `pytest .claude/scripts/tests/` で FAIL=0 | AC-7 |
...
> 実装前に `pytest .claude/scripts/tests/test_distill_lessons.py -v` を実行し、21 件の現状 PASS を確認した上でスキップ条件を追加すること。
