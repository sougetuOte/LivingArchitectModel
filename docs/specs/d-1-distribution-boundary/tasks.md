...
> **S を付けるとき、分割の中身を考えないこと**（体験シミュで検出 / 2026-07-27）
>
> T2 で決めるのは **P / X / S のどれか**だけである。**「どこを抜くか」は T4 の仕事**。
> S は「混在しているので後で分ける」という**保留の印**であって、分割案ではない。
>
> 該当箇所が多いファイル（`fable-l3-protocol.md` は 19 指令・`hga-summoning.md` は 9 指令）で
> 中身を考え始めると 1 件に時間を溶かし、**15 件を 1 セッションで終える前提が崩れる**。
> 崩れると中断が入り、基準が揺れる（design §4 が中断を禁じた理由そのもの）。
>
> **迷ったら S を付けて次へ進む。** S は後戻りできる判定である。
