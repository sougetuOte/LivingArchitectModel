...
**完了条件**:
- [ ] `_hook_utils.py` ファイルが存在
- [ ] 10関数全て実装: `get_project_root()`, `read_stdin_json()`, `get_tool_name()`, `get_tool_input()`, `get_tool_response()`, `normalize_path()`, `log_entry()`, `atomic_write_json()`, `run_command()`, `safe_exit()`
- [ ] `get_project_root()`: 環境変数 `LAM_PROJECT_ROOT` が設定されていれば優先（テスト用オーバーライド）
- [ ] 標準ライブラリのみ依存（外部パッケージなし）
- [ ] シバン行 `#!/usr/bin/env python3` なし（ユーティリティモジュールのため）
- [ ] Windows パス対応: `pathlib.Path` 一貫使用
- [ ] アトミック書き込み: `tempfile(dir=対象と同ディレクトリ) + os.replace` + exponential backoff retry (3回, 100ms/200ms/400ms)
- [ ] 型ヒント: Python 3.8 互換（`Optional[X]` 記法、`from __future__ import annotations` 使用可）
...
**完了条件**:
- [ ] `pre-compact.py` ファイルが存在
- [ ] シバン行 `#!/usr/bin/env python3` あり
- [ ] stdin JSON 読み込み不要（使用しない入力を自動ignore）
- [ ] フロー全実装:
  - [ ] PreCompact 発火フラグ書き込み（タイムスタンプ ISO 8601）
  - [ ] SESSION_STATE.md への記録（冪等処理）
  - [ ] lam-loop-state.json のバックアップ
- [ ] エラー耐性: 常に exit 0（圧縮をブロックしない）
- [ ] Windows パス対応: `pathlib.Path` 使用
- [ ] ファイル書き込み失敗時も回復（try-except で safe_exit）
- [ ] bash 版と同一の副作用（ファイル名・フォーマット一致）
- [ ] 実行権限付与: `chmod +x`（shebang 直接実行対応）
...
**完了条件**:
- [ ] `tests/__init__.py` ファイルが存在（空ファイル、pytest パッケージ認識用）
- [ ] `tests/test_hook_utils.py` ファイルが存在
- [ ] テストケース:
  - [ ] `test_get_project_root_default`: `__file__` ベースの PROJECT_ROOT 取得
  - [ ] `test_get_project_root_env_override`: `LAM_PROJECT_ROOT` 環境変数でのオーバーライド
  - [ ] `test_read_stdin_json_valid`: 正常な JSON 入力
  - [ ] `test_read_stdin_json_invalid`: 不正入力 → 空 dict
  - [ ] `test_normalize_path_absolute`: 絶対パス → 相対パス変換
  - [ ] `test_normalize_path_relative`: 相対パスはそのまま
  - [ ] `test_atomic_write_json`: JSON のアトミック書き込み + 読み戻し検証
  - [ ] `test_log_entry`: TSV ログ追記
  - [ ] `test_safe_exit`: exit code の検証
- [ ] `pytest .claude/hooks/tests/test_hook_utils.py -v` で全 PASS
- [ ] 外部パッケージ不要（pytest のみ）
...
- [ ] テスト環境セットアップ:
  - [ ] テンポラリプロジェクトディレクトリ作成
  - [ ] `.claude/logs/`, `.claude/` ディレクトリ作成
- [ ] 複数ケース実行:
  - [ ] 正常系: 入力 `{}` → exit 0 + ファイル作成
  - [ ] SESSION_STATE.md 既存時: 重複エントリなし（上書き）
  - [ ] lam-loop-state.json 既存時: バックアップ作成
- [ ] bash 版との出力比較:
  - [ ] exit code 一致
  - [ ] ファイル名・フォーマット一致
  - [ ] タイムスタンプ形式（ISO 8601）一致
- [ ] ドキュメント: テスト結果を `.claude/states/hooks-python-migration.json` に記録
...
```
- [ ] 動作確認後、方式 A を採用（方式 B は Windows フォールバックとして QUICKSTART に記載）
- [ ] 他の PreToolUse, PostToolUse, Stop は `.sh` のまま
- [ ] JSON 形式が有効（jq or Python json で validate）
...
**完了条件**:
- [ ] Claude Code の `.claude/settings.json` を新版に反映
- [ ] Claude Code を再起動（フック設定の再読み込み）
- [ ] 通常操作（Read → Edit など）を複数回実施
- [ ] コンテキスト圧縮が発火しないこと（PreCompact hook が正常実行されていることを確認）
- [ ] `.claude/pre-compact-fired` ファイルが作成されていることを確認
- [ ] `.claude/logs/` にログが記録されていることを確認
- [ ] エラー、例外が発生していないこと
...
**完了条件**:
- [ ] `tests/conftest.py` ファイルが存在
- [ ] 3つ以上の fixtures 定義:
  - [ ] `project_root(tmp_path)`: テンポラリプロジェクトルート作成
  - [ ] `hook_runner(project_root)`: フック subprocess 実行ヘルパー
  - [ ] `cleanup_files(project_root)`: テスト後クリーンアップ
- [ ] `run_hook()` ヘルパー関数:
  - [ ] subprocess.run で `sys.executable` フック実行（python3 ハードコード回避）
  - [ ] stdin JSON 入力対応
  - [ ] stdout, stderr, exit code 返却
  - [ ] タイムアウト 30秒設定
  - [ ] `env` パラメータで `LAM_PROJECT_ROOT` を `tmp_path` に設定（実プロジェクト汚染防止）
- [ ] Windows/Linux 両対応（pathlib.Path 使用）
- [ ] `tests/__init__.py` 存在確認（W1-T2a で作成済み）
...
**完了条件**:
- [ ] `tests/test_pre_tool_use.py` ファイルが存在
- [ ] テストケース実装（8個、設計書 TC-2〜TC-7 の6個 + 追加2個）:
  - [ ] `test_read_tool_pg_allow`: Read ツール → PG 許可（exit 0）（TC-2）
  - [ ] `test_edit_specs_pm_deny`: Edit docs/specs/ → PM deny（block JSON）（TC-3）
  - [ ] `test_edit_rules_auto_generated_pm_deny`: Edit .claude/rules/auto-generated/ → PM deny（TC-4）
  - [ ] `test_absolute_path_normalization`: 絶対パス → 相対パス正規化（TC-5）
  - [ ] `test_edit_src_se_allow`: Edit src/ → SE 許可（exit 0）（TC-6）
  - [ ] `test_log_truncation`: ログメッセージが 100 文字で trunc される（TC-7）
  - [ ] `test_glob_tool_pg_allow`: Glob ツール → PG 許可（追加）
  - [ ] `test_grep_tool_pg_allow`: Grep ツール → PG 許可（追加）
- [ ] TDD Red: 実装前にスケルトン `pre-tool-use.py`（`exit 0` のみ）を作成し、テストが意味のある失敗をすることを確認
- [ ] 各テストが conftest.py の fixtures を使用
- [ ] JSON 入出力の検証（json.loads で decode）
- [ ] ログファイル副作用の検証（ファイルが期待通り作成・追記される）
- [ ] Windows パス対応（`PurePosixPath` 正規化等）
...
**完了条件**:
- [ ] `pre-tool-use.py` ファイルが存在
- [ ] シバン行 `#!/usr/bin/env python3` あり
- [ ] 実装フロー:
  - [ ] stdin から JSON 読み込み（read_stdin_json）
  - [ ] tool_name 抽出
  - [ ] Read/Glob/Grep/WebSearch/WebFetch → PG 判定
  - [ ] file_path/command を抽出
  - [ ] normalize_path() でパス正規化（絶対→相対）
  - [ ] 正規表現パターンマッチ: docs/specs/, .claude/rules/auto-generated/, src/, docs/, その他
  - [ ] AUDITING フェーズの PG 級コマンド特別処理（PHASE_FILE を読み込み）
  - [ ] ログ記録（log_entry）: timestamp, level, source, message（100文字 trunc）
  - [ ] PM 級判定時: deny JSON を stdout に出力、exit 0
  - [ ] その他: exit 0（許可）
- [ ] エラー耐性: `try-except Exception` で全体をラップ + `finally` ブロック、例外時は exit 0
- [ ] Windows パス対応: `pathlib.Path` + `PurePosixPath` 正規化
- [ ] bash 版との完全パリティ（テストで検証）
- [ ] 実行権限付与: `chmod +x`
...
**完了条件**:
- [ ] テスト実行コマンド: `pytest .claude/hooks/tests/test_pre_tool_use.py -v`
- [ ] 全 8 テストが PASS
- [ ] カバレッジ: `pre-tool-use.py` の主要分岐が全て通る
- [ ] エラーメッセージなし
- [ ] ドキュメント: テスト結果を `.claude/states/hooks-python-migration.json` に記録
...
```
- [ ] Wave 1 の PreCompact 設定は維持（累積）
- [ ] PostToolUse, Stop は .sh のまま
- [ ] JSON 形式が有効
...
**完了条件**:
- [ ] Claude Code の設定を新版に反映
- [ ] 権限等級判定テスト（複数操作を実施）:
  - [ ] Read ツール（許可）: ファイル読み込み複数回 → ブロックなし
  - [ ] Edit docs/specs/ （拒否）: 仕様書編集 → block JSON が表示される
  - [ ] Edit src/ （許可）: ソースコード編集 → 実行許可が得られる
- [ ] `.claude/logs/permission.log` が記録されていることを確認
- [ ] エラー、例外が発生していないこと
...
**完了条件**:
- [ ] `tests/test_post_tool_use.py` ファイルが存在
- [ ] テストケース実装（10個、設計書 T1〜T6,T8〜T10 の9個 + 追加1個）:
  - [ ] `test_pytest_fail_recorded`: pytest 失敗 → tdd-patterns.log に FAIL 記録（T1）
  - [ ] `test_pytest_pass_after_fail`: 失敗後成功 → PASS 記録 + パターン追加（T2）
  - [ ] `test_edit_src_doc_sync_flag`: Edit src/ → doc-sync-flag に追記（T3）
  - [ ] `test_write_src_doc_sync_flag`: Write src/ → doc-sync-flag に追記（T4）
  - [ ] `test_edit_docs_no_sync_flag`: Edit docs/ → flag 追記なし（T5）
  - [ ] `test_loop_state_tool_events`: lam-loop-state.json 存在時 → tool_events に追記（T6、jq 依存除去確認含む）
  - [ ] `test_npm_test_fail_recorded`: npm test 失敗 → 記録（T8）
  - [ ] `test_go_test_fail_recorded`: go test 失敗 → 記録（T9）
  - [ ] `test_non_test_command_no_record`: テスト以外コマンド → 記録なし（T10）
  - [ ] `test_atomic_write_safety`: JSON アトミック書き込みが正常（追加、_hook_utils.py を直接 import してテスト）
- [ ] TDD Red: 実装前にスケルトン `post-tool-use.py`（`exit 0` のみ）を作成
- [ ] PostToolUseFailure 検証: 既存 bash 版で pytest 失敗時のイベントタイプを実機確認。必要に応じて W3-T4 に PostToolUseFailure 登録を追加
- [ ] 各テストが conftest.py の fixtures を使用（`LAM_PROJECT_ROOT` 環境変数で実プロジェクト汚染防止）
- [ ] JSON ファイル副作用の検証（json.load で decode）
- [ ] 重複排除ロジック検証（同一エントリが二重記録されないこと）
...
**完了条件**:
- [ ] `post-tool-use.py` ファイルが存在
- [ ] シバン行 `#!/usr/bin/env python3` あり
- [ ] 実装フロー:
  - [ ] stdin から JSON 読み込み（tool_name, tool_response）
  - [ ] テストコマンド判定（pytest/npm test/go test）
  - [ ] tool_response で exit code 判定（0 = 成功, != 0 = 失敗）
  - [ ] 失敗時: tdd-patterns.log に FAIL レコード追加
  - [ ] 成功かつ前回失敗: PASS レコード追加 + パターンマッチング
  - [ ] Edit/Write + src/ パス → doc-sync-flag に追記（重複排除: set で管理）
  - [ ] lam-loop-state.json 存在時 → tool_events に追記（atomic_write_json）
- [ ] エラー耐性: `try-except Exception` + `finally` で常に exit 0
- [ ] クロスプラットフォーム: pathlib.Path で パス判定
- [ ] ログ形式: 既存 bash 版と同一（TSV, タイムスタンプ ISO 8601）
- [ ] 実行権限付与: `chmod +x`
...
**完了条件**:
- [ ] テスト実行コマンド: `pytest .claude/hooks/tests/test_post_tool_use.py -v`
- [ ] 全 10 テストが PASS
- [ ] JSON ファイル操作（読取・書込・アトミック化）が正常
- [ ] ドキュメント: テスト結果を `.claude/states/hooks-python-migration.json` に記録
...
- [ ] Claude Code の設定を新版に反映
- [ ] TDD パターン検出テスト:
  - [ ] pytest 失敗コマンド実行 → tdd-patterns.log に FAIL 記録される
  - [ ] 失敗後成功実行 → PASS 記録される
- [ ] doc-sync フラグテスト:
  - [ ] Edit src/ 実行 → doc-sync-flag ファイルに追記される
  - [ ] Edit docs/ 実行 → doc-sync-flag に追記されない
- [ ] ループログテスト（ループ中の場合）:
  - [ ] lam-loop-state.json に tool_events が追記される
- [ ] ログファイル構造が正常（TSV 形式）
...
**完了条件**:
- [ ] `tests/test_stop_hook.py` ファイルが存在
- [ ] テストケース実装（6個）:
  - [ ] `test_no_state_file_stops`: 状態ファイルなし → 停止（exit 0）
  - [ ] `test_max_iterations_stops`: 上限到達 → 停止
  - [ ] `test_recursion_guard`: 再帰防止チェック（stop_hook_active=true）
  - [ ] `test_test_fail_blocks`: テスト失敗 → block JSON 出力
  - [ ] `test_precompact_stops`: PreCompact 発火フラグ存在 → 停止
  - [ ] `test_state_schema_valid`: lam-loop-state.json スキーマ正当性
- [ ] TDD Red: 実装前にスケルトン `lam-stop-hook.py`（`exit 0` のみ）を作成
- [ ] テスト環境設計:
  - [ ] レベル1（ブラックボックス）: subprocess でフック全体を実行。`LAM_PROJECT_ROOT` で隔離
  - [ ] レベル2（ユニット）: subprocess.run を mock し、pytest/ruff 実行を模擬
  - [ ] `tmp_path` に最小テスト環境（Makefile 等）を構築
- [ ] JSON スキーマ検証（必須フィールド確認）
- [ ] Stop フック出力: `"block"` のみが有効な decision 値。停止許可時は JSON を出力しない
...
**完了条件**:
- [ ] `lam-stop-hook.py` ファイルが存在
- [ ] シバン行 `#!/usr/bin/env python3` あり
- [ ] 実装フロー（7ステップ）:
  - [ ] 再帰防止チェック（stop_hook_active フラグ）
  - [ ] 状態ファイル確認（lam-loop-state.json 存在チェック）
  - [ ] 反復上限チェック（iterations > max_iterations → 停止）
  - [ ] PreCompact 発火フラグによるコンテキスト残量チェック
  - [ ] Green State 判定:
    - [ ] テスト自動検出（pytest/npm test/go test 実行 → exit code 0）
    - [ ] lint 自動検出（ruff/eslint 実行 → 指摘なし）
    - [ ] セキュリティチェック（bandit 実行 → 指摘なし）
  - [ ] エスカレーション条件チェック:
    - [ ] テスト数減少（前回比）
    - [ ] Issue 再発（フラグファイル確認）
  - [ ] 継続 or 停止判定
- [ ] JSON 出力: 継続時は `{ "decision": "block", "reason": "..." }`、停止許可時は出力なし（exit 0 のみ）
  - [ ] 注: `"block"` は唯一の有効な decision 値（公式仕様準拠）
- [ ] subprocess 実行:
  - [ ] `subprocess.run` でコマンド実行（shutil.which で解決）
  - [ ] タイムアウト制御（subprocess timeout パラメータ）
  - [ ] shell=False 固定（セキュリティ）
- [ ] エラー耐性: `try-except Exception` + `finally` で常に exit 0（フック障害でループをブロックしない）
- [ ] 実行権限付与: `chmod +x`
...
**完了条件**:
- [ ] テスト実行コマンド:
  - `pytest .claude/hooks/tests/test_stop_hook.py -v`
  - `pytest .claude/hooks/tests/test_loop_integration.py -v`
- [ ] 全 11 テストが PASS
- [ ] subprocess mock が正常（コマンド実行シミュレーション）
- [ ] 状態遷移ロジックが正常
- [ ] ドキュメント: テスト結果を `.claude/states/hooks-python-migration.json` に記録
...
```
- [ ] Wave 1-3 の設定は全て .py に（PreCompact, PreToolUse, PostToolUse）
- [ ] JSON 形式が有効
- [ ] 全 4 フック設定が完了した状態
...
**完了条件**:
- [ ] Claude Code の設定を新版に反映（全フック .py に切替完了）
- [ ] 複合テスト（全フック連携）:
  - [ ] 権限判定（PreToolUse）
  - [ ] パターン記録（PostToolUse）
  - [ ] ループ停止判定（Stop hook）
- [ ] ループ自動停止テスト:
  - [ ] テスト成功 → Stop hook で停止判定 → ループ終了
  - [ ] コンテキスト枯渇 → PreCompact 発火 → Stop hook で停止
- [ ] エラー、例外が発生していないこと
- [ ] ログファイル（permission.log, tdd-patterns.log, loop.log）が正常記録
...
**完了条件**:
- [ ] テスト実行コマンド: `pytest .claude/hooks/tests/ -v`
- [ ] 全テストが PASS
  - [ ] test_hook_utils.py (9個)
  - [ ] test_pre_tool_use.py (8個)
  - [ ] test_post_tool_use.py (10個)
  - [ ] test_stop_hook.py (6個)
  - [ ] test_loop_integration.py (5個)
- [ ] カバレッジ: 各フックの主要分岐が全て通る（pytest --cov で確認推奨）
- [ ] 外部パッケージ依存なし（pytest のみ。ランタイムは標準ライブラリ）
...
**完了条件**:
- [ ] ファイルパス: `./requirements-dev.txt` （プロジェクトルート）
- [ ] 内容:
```
# LAM hook 開発・テスト用
pytest>=7.0
...
**完了条件**:
- [ ] 日本語版更新（QUICKSTART.md）:
  - [ ] セクション: Q: Python は必須？
  - [ ] A: 必須です。フックと StatusLine が Python 3.8+ を使用
  - [ ] uv でのセットアップ手順（推奨）
  - [ ] venv でのセットアップ手順（フォールバック）
  - [ ] 他のツール（pyenv, conda）への言及
- [ ] 英語版も同様に更新（QUICKSTART_en.md）
- [ ] ドキュメント内の「Windows での python3 コマンド」注記を追加
...
**完了条件**:
- [ ] 日本語版更新（README.md）:
  - [ ] 前提条件セクション（Prerequisites）に以下を追加:
    ```
    - Python 3.8+（フック・StatusLine に必要）
    ```
- [ ] 英語版も同様に更新（README_en.md）
- [ ] 他の前提条件（Git, Node.js 等）と並列記載
...
- [ ] 実行権限確認: `ls -la .claude/hooks/*.py` で `-rwxr-xr-x` （実行権限あり）を確認
- [ ] 権限なし場合: `chmod +x .claude/hooks/*.py` で付与
