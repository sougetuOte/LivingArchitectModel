...
現行 MVP 実装（`lam-stop-hook.py` `_save_loop_log()`）の実出力フォーマット
（2026-06-11 B-2 監査 PM4-1 で実装に合わせて更新）:
...
--- Iteration Log ---
iter 1: found=6 fixed=5 pg=3 se=2 pm=1 tests=120
iter 2: found=2 fixed=2 pg=1 se=1 pm=0 tests=122
iter 3: found=0 fixed=0 pg=0 se=0 pm=0 tests=122
```

イテレーション部は `lam-loop-state.json` の `log[]` エントリを 1 行/イテレーションで
出力したものであり、フィールドの意味は §3.2 のマッピング表に従う
（found/fixed/pg/se/pm/tests = `issues_found`/`issues_fixed`/`pg`/`se`/`pm`/`test_count`）。

#### 完全実装での目標形式（参考・未実装）

重要度別内訳（Critical/Warning/Info）・Actions・Green State 判定の構造化記録は
完全実装（JSON 移行・§2 スキーマ）のスコープ。テキスト形式の中間段階として
以下の構造化フォーマットを採用する選択肢もあったが、MVP では grep 集計しやすい
1 行形式を採用した:
