...
- バージョン: 1.2.0
- 作成日: 2026-06-11
- 改訂日: 2026-06-12（W0 結果反映: W0-T2 起票・W2-T1 検証手段追記・DW 置き換え見送り記録・Plan B 確定反映。PM 承認済み）
- 改訂履歴: 2026-06-12 v1.1.0（design v0.3.0 / requirements v1.2.0 整合対応）
- ステータス: Draft（PM 承認待ち）
- 参照要件: `requirements.md` v1.2.0
- 参照設計: `design.md` v0.3.0
...
| W4 | メモリ蒸留 + コスト集計 | 2 | distill-lessons.py / cost-log |
| W5 | full-review 連携（PM-G2 承認後） | 2 | PM ゲート通過後のみ着手可 |
| W6 | lam-orchestrate 接続 + DW 検証タスク | 2 | SE 級 |
| W7 | 独立 grader 検収 + デモ取得 + 最終承認 | 3 | NFR-8 / PM-G3（W7-T3 新規） |

**総タスク数（定義）**: 20（PM ゲート 3 件を含む）  
**総タスク数（実施）**: 19（W3 は W3-T2 のみ実施・Plan B 確定 2026-06-12）  
...
**完了条件**:
- [ ] PM が「承認」と回答している
- [ ] 承認範囲（B-3 節のコード・競合排除条件）が明記されている
...
- [ ] スモークテスト: 同一セッション内での起動確認は最小 DW プローブ（**開発時テストハーネス用途**・FR-8 のデフォルト実行経路排除とは別物・`research/dw-replacement-analysis.md §2 A5`）で実施してよい。新セッションでの確認・静的 pytest も代替手段として有効
...
**完了条件**:
- [ ] SKILL.md スクリプトに Plan B 制御ループが実装されている
- [ ] l3-executor と grader がそれぞれ独立した Agent 呼び出しで起動される（AC-5）
- [ ] grader 判定 JSON を受け取り、合格/不合格/エスカレーションに分岐する
- [ ] ネスト失敗フォールバック（design §11b: 三層→二層退避）のエラー検知が実装されている
- [ ] `gd-session-state.json` の `fallback: "two_layer"` セットが実装されている
- [ ] pytest: Plan B ループの単体テスト（モック grader 使用）がパスする
- [ ] grader がエラーまたは不正 JSON を返した場合に 1 回のみ再試行し、再試行も失敗した場合はエスカレーションする処理が実装されている（design §8 MUST）。grader 失敗を合格として扱う経路が存在しない（MUST NOT）
- [ ] Plan B 採用理由（`/goal` を使用しない根拠）が `design.md §8` と `research/oq1-goal-subagent-test.md` に記録されている（NFR-2 MUST）
...
**完了条件**:
- [ ] `.claude/scripts/distill-lessons.py` が存在する
- [ ] 検証済みの教訓は `.claude/agent-memory/goal-driven-l3-executor/lessons.md` にのみ書き込む
- [ ] 未検証の推測は「未検証」タグ付きで同ファイルに追記する（MUST）
- [ ] `docs/artifacts/knowledge/` への書き込みコードが存在しない（W-5 制約 / design §13）
- [ ] `lessons.md` エントリ形式が design §13 のスキーマに準拠している
- [ ] **SKILL.md フロー[7] からの distill-lessons.py 呼び出し配線が記載されており、テストで検証されている**（前提4）
- [ ] pytest: 蒸留結果の書き込み先・フォーマット検証テストがパスする
- [ ] 小タスクルートでは `distill-lessons.py` の入力が grader 判定 JSON のみとなる処理が実装されている（design §9.1: L1 最終検収をスキップするため）
...
**完了条件**:
- [ ] Agent ツール結果の `subagent_tokens` を読み取り、層別（L1/L2/L3/grader）に累積する処理が実装されている
- [ ] タスク完了時に design §14 のコストサマリ形式で出力される（AC-11）
- [ ] **`l1_ratio = l1_tokens / total_tokens` が計算・出力されている**（分母は `gd-session-state.json` トップレベルの `total_tokens`。design §14 正規フィールド定義。参考記録・20% 以下は初期目標値・SHOULD であり、未達でもタスク完了）
- [ ] 各 Agent 呼び出し後に `gd-session-state.json` トップレベルの `total_tokens` が更新される（bound 判定の正規フィールド。`cost_log` は層別内訳のみ保持し合計値を重複保持しない。design §14）
- [ ] `subagent_tokens` 取得失敗時の推定値フォールバックが実装されている（P-2 対応）
- [ ] pytest: コスト集計ロジックの単体テストがパスする
...
**Orphan 確認**: 全タスク（20 定義）が FR/NFR/AC にトレース可能。Orphan なし。
...
| AC-11 | コストサマリ出力確認 | W4-T2（実装・テスト）/ W7-T2（デモ確認）/ W1-T3（6/22 実機確認） |
...
*tasks.md ここまで。バージョン 1.0.0 / 2026-06-11 / task-decomposer サブエージェント*
