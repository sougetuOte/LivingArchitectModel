...
- 作成日: 2026-06-11
- ステータス: Draft（PM 承認待ち）
- 参照要件: `requirements.md` v1.1.0
- 参照設計: `design.md` v0.2.0
...
4. 同時に `/goal` 条件文でのトークン閾値書式（`stop if total_tokens > N`）の有効性も確認（C-4 関連）
...
- [ ] `design.md §8` に「確定: Plan A」または「確定: Plan B」の注記が追記されている
...
- [ ] エスカレーション E2E テスト（AC-9）: 常に fail を返す grader スタブ + `loop_count=max` の状態ファイルで、エスカレーション報告が出力されることを pytest で確認（骨格レベル・Plan A/B 共通に成立）
...
- [ ] **`l1_ratio = l1_tokens / total_tokens` が計算・出力されている**（参考記録。20% 以下は初期目標値・SHOULD であり、未達でもタスク完了）
