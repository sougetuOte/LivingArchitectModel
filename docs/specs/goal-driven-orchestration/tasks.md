...
- バージョン: 1.1.0
- 作成日: 2026-06-11
- 改訂日: 2026-06-12（design v0.3.0 / requirements v1.2.0 整合対応）
- ステータス: Draft（PM 承認待ち）
...
| Wave | 内容 | タスク数 | 備考 |
|------|------|---------|------|
| W0 | Spike: OQ-1 実測検証 | 1 | 全内側ループ実装の先行必須。Plan A/B 分岐点 |
...
**総タスク数（定義）**: 19（PM ゲート 3 件を含む）  
**総タスク数（実施）**: 18（W3 は Plan A または B の 1 件のみ実施）  
...
**注記**: Plan B 確定時（W0-T1 で /goal がサブエージェント内で動作しない場合）、
W1-T1/W3 の規模が M→M+ に増える可能性がある（自前ループの実装コスト増）。
...
**Orphan 確認**: 全タスク（19 定義）が FR/NFR/AC にトレース可能。Orphan なし。
