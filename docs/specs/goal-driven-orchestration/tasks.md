...
**完了条件**:
- [x] 大タスクルート判定（工程数 ≥ 3 または並列分解必要）がログで確認できる
- [x] l2-foreman → l3-executor の工程分配が実機で動作する
- [x] 並列予算チェック（design §10: サブ予算合計 ≤ 残予算、不成立時は順次起動に退避）の実行記録が確認できる
- [x] §11b 三層→二層フォールバックの実機確認（ネスト失敗の人工誘発が困難な場合は pytest 結果での代替を許容し、その判断理由を記録する）
- [x] 実行ログ・コストサマリ（層別: L1/L2/L3/grader）が `docs/artifacts/goal-driven-demo/` に保存されている
- [x] 大タスク bound 初期値（400k/7200s）の妥当性が実測データで評価されている

**2026-06-18 完了**（大タスク三層実機完走・grader 7/7 Pass・Critical=0・fallback=null・cost total 196,139 / bound 400k の 49%。コミット: 09ddd05）
...
**完了条件**:
- [x] W7-T1 の検収結果（grader 合否 JSON）が PM に提示されている
- [x] PM が Critical / Warning / Info の項目数を確認している
- [x] Critical = 0 かつ Warning = 0 で最終合格と判定されている
- [x] PM が「承認」と回答している

**2026-06-18 PM 承認**（最終合格判定）。承認範囲: W7-T1 検収結果（grader 25/25 Pass / Critical=0 / Warning=0）+ W7-T2 evidence（AC-3/4/5/11 全 Pass）+ W7-T2b evidence（大タスク三層実機 / grader 7/7 Pass / Critical=0）で **B-3 マイルストーン最終合格**
...
*tasks.md ここまで。バージョン 1.4.6 / 2026-06-18 / Sonnet 実行層（W7-T2b 完了チェック記入 + PM-G3 承認反映。B-3 マイルストーン完了）*
