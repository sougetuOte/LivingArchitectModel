...
| `status` | string | `"running"` | セッション状態。取りうる値: `"running"` / `"escalated"` / `"completed"`。`"escalated"` は bound 超過またはループ上限到達時、`"completed"` は grader 合格による正常完了時（design §10 参照） |
| `fallback` | string \| null | `null` | 二層フォールバック方式の記録。取りうる値: `null` / `"two_layer"`。三層→二層退避発動時（design §11b）に `"two_layer"` が設定される |
...
*config.md ここまで。バージョン 1.0.1 / 2026-06-13 / doc-writer サブエージェント（W1-T2, 2026-06-13 更新）*
