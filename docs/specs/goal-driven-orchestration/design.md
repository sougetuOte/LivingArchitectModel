...
|-----------|--------|--------|--------|
| L3 打ち切りターン数（`/goal` 条件文） | 10 | 20 | 15（工程ごと） |
...
| 防衛線 | 主体 | 方式 |
|--------|------|------|
| **第一（主）** | スキルスクリプト（L1 コンテキスト） | 各 Agent 呼び出し後に Agent ツール結果の実測 `subagent_tokens` を加算し（取得失敗時のみ自己申告 `tokens_used` を P-2 フォールバックとして採用）、**次の spawn 前に残予算チェック**（spawn-time enforcement） |
...
|----|-----------|------------------------|------------|
| L3 実行ループ（第一） | `/goal` 条件文の打ち切り句（C-4 対応: ターン数のみ） | 小:10 / 中:20 / 大:15 | 構造化報告に `next_suggestion: "ESCALATE"` を設定 |
| L3 実行ループ（バックストップ） | `maxTurns` フロントマター | **20 固定**（ルート別に変えられない / 下記注記） | Claude Code がターンを打ち切る（`max_turns_reached`） |
| グローバル | 総トークン OR 経過時間 | 小:50k/3600s, 中:150k/3600s, 大:400k/7200s | スクリプト（第一）or Stop hook（第二）でエスカレーション |

> **`maxTurns` フロントマターがルート別にならない理由（2026-08-21 実測）**: frontmatter は静的であり、
> L3 の agent 定義は `.claude/agents/goal-driven-l3-executor.md` の **1 ファイルのみ**であるため、
> 呼び出しごとにルート別の値を与えることはできない（`Agent` ツールに `maxTurns` パラメータはない）。
> よって frontmatter には上表の最大値 **20** を固定で置き、**どのルートでも必ず止まる最終防波堤**として
> 扱う。ルート別の締め付け（10 / 20 / 15）は、**Plan A（`/goal` 使用）では**条件文が担う。
>
> **Plan B（自前ループ / これが現行実装）ではルート別の粒度は存在しない。** Plan B は `/goal` を
> 使わないため打ち切り句そのものがなく（`.claude/skills/goal-driven/references/route-and-bound.md`
> §打ち切り制御）、ターン数による打ち切りは **`maxTurns` 20 のみ**が担う。したがって小タスク
> （設計値 10）でも 20 ターンまで走る。これは設計意図より緩いが、**改訂前はターン数打ち切りが
> 一切効いていなかった**ため後退ではない。ルート別の締め付けを Plan B でも取り戻すには L3 agent
> 定義のファイル分割が要る（本改訂では見送り / 判断は 2026-08-21 のユーザー承認による）。
>
> **キー名の訂正**: 本設計はかつて `max_turns`（snake_case）と記していたが、これは
> subagent frontmatter の有効キーではなく、**bound は一度も機能していなかった**
> （2026-08-20 の処方監査で発覚）。正しいキーは **`maxTurns`（camelCase）**である。
> 実測記録: `docs/artifacts/maxturns-probe-2026-08-21.md`。
...
**正規フィールドの定義**: グローバル bound 判定の正規フィールドは `gd-session-state.json` **トップレベルの `total_tokens`**（§10）であり、Stop hook B-3 節もこれを参照する。`cost_log` は層別内訳のみを保持し、合計値を重複保持しない（二重更新による bound 不全を防ぐ）。`l1_ratio` の分母はトップレベル `total_tokens`（`compute_l1_ratio()` 関数で計算）。

**累積ソースの優先順位**:
1. **実測値優先**: Agent ツール結果の `subagent_tokens` を正規ソースとして採用する（MUST）。
2. **P-2 フォールバック**: `subagent_tokens` が `None` の場合のみ、executor / grader の自己申告 `tokens_used` を採用し、WARN ログを出力する。

**乖離検知ルール（W4-T2 追加）**:
- 乖離率 = `abs(measured - self_reported) / max(measured, 1)`
- 乖離率が **0.20 を超えた場合**: stdout に `[gd-warn] token divergence` メッセージを出力し、`cost_log._divergences` 配列に乖離情報（layer / self_reported / measured / ratio）を記録する
- 乖離率閾値 0.20 は初期固定値。運用データ蓄積後に config 外部化を検討する（Open Question）

> **背景**: W1-T3 実機観測で executor 自己申告 100 token に対し Agent ツール実測 26,305 token
> という大幅な乖離が観測された。実測優先設計の根拠となるデータポイント。
...
| P-2 | Agent ツール結果に subagent_tokens が安定して返る | N=1 実測（ファクトシート §5 確認済み）。多層・並列時は未検証 | 実装時の検証項目に追加。**取得失敗時は自己申告 `tokens_used` 値を採用（PM 指示 2026-06-13）**。WARN ログで通知し `cost_log._divergences` に記録する。`subagent_tokens` の取得単位（累計か単一スパン分か）も実装時検証項目に含める。（旧定義「spawn 数×平均値でフォールバック」は PM 指示により本定義に変更）|
...
*設計書ここまで。バージョン 0.3.3 / 2026-06-16 / W4-T2 同期（§10 bound 正規ソース・§14 乖離検知・§19 P-2 フォールバック定義変更）*
