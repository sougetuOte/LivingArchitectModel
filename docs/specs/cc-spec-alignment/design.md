...
commands は **Claude が自動起動しない**が、skills は **自動起動しうる**（`disable-model-invocation` で制御）。
これは単なる量的移行ではなく、移行作業の核は **各コマンドの起動方針を1本ずつ判定すること**:

> **書式の事後注記（2026-05-29, Wave 3 裏取り）**: 本節の初稿は `user-invocable` を制御キーとして併記していたが、
> context7 での裏取りの結果、独立した `user-invocable` キーは存在せず、**`disable-model-invocation: true`**
> が「ユーザー起動のみ・モデル自動起動なし」を表現する正式書式であると確定した。
> 実装（T3-1〜T3-11）および判定表（[wave3-migration-table.md](wave3-migration-table.md) §2）は正しい書式に基づく。
> 教訓は `.claude/rules/upstream-first.md`「設計フェーズへの適用」に反映済み。
...
## 7.6. Wave 2 採用実績（2026-05-28 確定・S-4 仕様同期）

§7 の計画に対する実装実績。採否は機能の実在性（裏取り済み）と **LAM 設計思想への適合性** の両面で判断した。

| 優先 | 機能 | 結果 | 根拠 |
|:----:|------|:----:|------|
| 高 | `paths`（自動起動） | **採用** | adr/spec/ui-design-guide に自動適用パスを宣言（T2-1, コミット 6069068） |
| 高 | `permissionDecision: "defer"` | **見送り** | MAGI 合議（[2026-05-28-magi-t2-2-defer.md](../../artifacts/2026-05-28-magi-t2-2-defer.md)）。PM級 ask の defer 化は「人間承認必須」憲法を settings 設定ミスで後退させうる。LAM は曖昧性を等級判定で吸収する設計のため defer の設計思想と不整合。diff P-2「現状維持で安全」と一致 |
| 中 | `model` 上書き | **採用** | テンプレート系3スキルに `model: sonnet` を委譲（T2-3, コミット 6069068）。magi/clarify/lam-orchestrate/skill-creator は inherit 維持 |
| 中 | `when_to_use` / `allowed-tools` | **採用** | 全7スキルに `when_to_use`、テンプレート系3スキルに最小 `allowed-tools`（Read/Write/Edit）を付与（T2-4, コミット 6069068） |
| 低→昇格 | `isolation: worktree` / `background` | **見送り** | `background` は権限プロンプト自動拒否が LAM の ask ガードと衝突・常駐不要。`worktree` は origin/master 分岐で未コミット変更が不可視となりレビュー/実装の主目的と矛盾。V-7 の昇格判断はこの副作用を見落としていた |
| 低→昇格 | `effort.level` / 新 hook ハンドラ | **見送り** | 新ハンドラは現行 command 型の確実性・フェイルセーフを損ない、prompt 型は等級判定を非決定化。effort.level は明確な活用箇所がなく行動変化を生まない。現行 hooks は Zero-Regression で無変更 |

> 計画段階（§7）では機能の実在性を裏取り（V-1〜V-8）したが、LAM 適合性の評価が未実施だった。
> 昇格組3つ（defer/worktree・background/effort.level）が軒並み見送りとなった所見は retro 記録対象。
