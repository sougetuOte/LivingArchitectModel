...
| G4: Wave 6 達成値の退行防止 | Lighthouse Accessibility ≥ 95 / CSS ≤ 10,240 bytes / pytest 全件 PASS | Stage 4 で全項目達成 |
...
### NFR-W7-1: CSS 予算管理（MUST）

Wave 7 で CSS を追加する場合、追加前に CSS 合計サイズを事前計測し、上限内に収まることを確認する。

**MUST 項目**:
- 上限: **10,240 bytes**（Wave 6 NFR-W6-3 継承）
- Wave 6 終端時点の実測: 9,922 bytes（残 318 bytes）
- Stage 3（V-2 複数 Milestone 一覧化）で CSS 追加が発生する場合、設計段階で残予算内に収まる根拠を design.md に明記
- 上限超過時は意匠の縮退 or 要件の縮小で対応（Wave 8+ への切り出しを優先検討）

> retro Wave 6 Try A4 の早期回収。
...
### AC-W7-5: CSS 予算遵守（NFR-W7-1 対応 / MUST）

dashboard.html 生成後の CSS 合計サイズが **10,240 bytes 以下**である。
