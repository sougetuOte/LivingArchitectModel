...
- [ ] **T-S3-3**: CSS 合計サイズ ≤ **16,384 bytes（16 KiB / v0.2.4 改訂）**（AC-W7-5 / SHOULD / 緑帯 < 11,469 bytes が望ましい / 赤帯 ≥ 14,746 bytes 到達時のみ design.md §8 縮退オプション適用）
...
- AC-W7-4 / AC-W7-5 達成
- pytest 全件 PASS
- CSS 予算遵守（**design.md §8 v0.2.4 事前評価 = ~10,362 bytes 想定 / 上限 16,384 bytes / 残 ~6,022 bytes / 緑帯 / 実測値で確認**）
...
| リスク | 対応 |
|:------|:----|
| CSS 予算オーバー（v0.2.4 改訂後は残 ~6,022 bytes / 緑帯） | T52 で実測 → 黄帯（70-90%）到達時は Wave 開始時の追加見込みを記録 → 赤帯（≥ 90%）到達時は retro で予算改定議題必須化 |
...
- [ ] **T-S4-4**: CSS サイズ実測 ≤ **16,384 bytes（v0.2.4 改訂）**（AC-W7-5 / SHOULD / Stage 3 で達成済の追認）
...
- [x] W7-B5-T51: V-2 セクション化（Milestone 名昇順ソート） @sonnet
- [x] W7-B5-T52: CSS 追加（実測必須） @sonnet
- [x] W7-B5-T53: Stage 3 自動テスト追加（モックフィクスチャ） @sonnet
- [x] W7-B5-T54: Stage 4 統合テスト @sonnet
- [x] W7-B5-T55: L1 Lighthouse 計測 + ユーザー PoC レビュー @human
...
- 完了条件:
  - design.md §8 の 3 ルール追加（合計 ~440 bytes 推定 / v0.2.4 改訂）
  - **追加前後の CSS 合計サイズを実測 → L1 報告**
  - 上限 **16,384 bytes（16 KiB / v0.2.4 改訂）** 内（緑帯 < 11,469 bytes が望ましい）
- 検証: ファイルサイズ計測（`wc -c` 等 / または `_render_style()` 戻り値の `len(s.encode("utf-8"))` 実測）
- 規模: S
...
| **0.2.4** | 2026-06-28 | NFR-W7-1 CSS 予算根本見直し連動補追 — §3 Stage 3 T-S3-3 + Stage 3 ゲート条件 + リスク表 + Stage 4 T-S4-4 を新上限 16,384 bytes 反映 + 3 段階レベル制（緑/黄/赤）表記に更新 / §6 T52 完了条件を 16,384 bytes + 推定 ~440 bytes に更新 / §9 v0.2.4 PM 補追承認記録 / §10 参照を v0.2.4 に更新 / requirements.md NFR-W7-1 + AC-W7-5 + G4 + design.md §8 + execution-plan.md §6 §8 と連動 / 議事録: `2026-06-28-magi-nfr-w7-1-budget-revision.md` |
...
Wave 7 Stage 3 BUILDING 着手時の CSS 予算逼迫（残 18 bytes 想定）を契機として、L1 提起の AoT 5 Atom MAGI 合議で NFR-W7-1 を根本改定。本タスク文書への補追:

- §3 Stage 3 T-S3-3 / リスク表 / Stage 3 ゲート条件 / Stage 4 T-S4-4 を新上限 16,384 bytes + 3 段階レベル制反映に更新
- §6 T52 完了条件を新上限 + 推定 ~440 bytes（CSS minify 不要前提）に更新
- §8 改訂履歴 v0.2.4 行追加
- §10 参照を v0.2.4 に更新

requirements.md NFR-W7-1 + AC-W7-5 + G4 + design.md §8 + execution-plan.md §6 / §8 と連動補追。
...
- [Wave 7 design.md](design.md) v0.2.4 Approved
