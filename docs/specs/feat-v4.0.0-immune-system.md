...
| 操作 | デフォルト最大回数 | 理由 |
|------|------------------|------|
| full-review ループ | 3回 | 3回で直らないものは人間判断が必要 |
...
| 観測回数 | 状態 | アクション |
|---------|------|-----------|
| 1回 | 観測（ノイズの可能性） | `docs/memos/tdd-patterns.md` に記録のみ |
...
**受入条件**:
- [ ] 失敗パターンは `docs/memos/tdd-patterns.md` に記録される
...
| | MVP | 完全実装 |
|-|-----|---------|
| 観測 | 手動で tdd-patterns.md に記録 | TDDサイクルからの自動抽出 |
...
- [ ] `tdd-patterns.md` のデータ構造とパターンマッチングロジック
