...
> **正式な要件定義書は `v4.0.0-immune-system-requirements.md`** を参照してください。
...
| 観測回数 | 状態 | アクション |
|---------|------|-----------|
| 1回 | 観測（ノイズの可能性） | `docs/artifacts/tdd-patterns/` にパターン記録ファイルを作成 |
...
**受入条件**:
- [ ] 失敗パターンは `.claude/tdd-patterns.log`（サマリー）および `docs/artifacts/tdd-patterns/`（詳細）に記録される
...
| | MVP | 完全実装 |
|-|-----|---------|
| 観測 | `.claude/tdd-patterns.log` + `docs/artifacts/tdd-patterns/` に記録 | TDDサイクルからの自動抽出 |
