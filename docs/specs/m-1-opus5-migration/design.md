...
ADR-0011 §実装計画 W0 が定める「削減台帳の器を作成（単純表で開始）」を、`docs/artifacts/m-1-clause-ledger.md` にヘッダ行のみのスケルトンとして作成する。列構成は §6.3 で確定する **8 列**（条項ID / 原文 / 出典（ファイル:節） / 判定軸（軸1〜4 + 基質適合） / 判定 / 移動先 / 監視条件 / ロード条件 / 判定日）とし、W0 時点ではヘッダ行のみを持つ空表とする（2026-07-25 に 7 列から改訂 / HGA #17）。データ行の記入は W2（トリアージ適用時）に行う。
...
上記いずれにも該当しない場合:
  ...
  │
  ├ 【基質適合テスト】「Opus 5 は指示なしでこれをやるか」= YES
  │    → 軸 4 の実測発火は Opus 4.x 以前の基質での観測であり、
  │      新基質での必要性の証拠として無効。 削減 または 運用移管
  │
  ├ 軸4 = 実測発火あり（基質適合テストを通過した場合のみ）
  │    ├ 軸2 = 列挙            → 圧縮（意図 1 行に畳む / 根拠は docs/artifacts へ退避）
  │    └ 軸2 = 意図            → 保全
  └ 軸4 = 発火ゼロ
       ├ 軸2 = 列挙            → 削減 または 運用移管
       └ 軸2 = 意図            → 削減 または 運用移管
                                 （保全とする場合は理由 1 行を必須とする / 下記「立証責任の所在」）
```

#### 決定木の出力（6 値の閉集合 / 2026-07-25 改訂）

| 出力 | 意味 |
|:-----|:-----|
| 保全 | 条文をそのまま残す |
| 圧縮 | 意図 1 行に畳み、根拠は `docs/artifacts/` へ退避する |
| 削減 | 条文を削除する（懸念ごと消す = リスクが実在しないと判定した場合） |
| SSOT 退避 | 別の SSOT へ移動する（削除ではない） |
| **運用移管**（2026-07-25 追加） | **機構（hook / test / rule 条文）で担保するのをやめ、運用（人の注意・retro での確認）へ明示的に移す**。条文は消えるが**リスク受容の記録は残る** |
| 対象外 | 不変制約 4 対象に該当し 4 軸評価を経ていない |

#### 「運用移管」の必須要件（FR-7 / 2026-07-25 追加）

「削減」との違いは、**リスクの存在を認めたうえで機構化しないと決めた**点にある。リスクを機構なしで受容する決定は**それ自体が軸 1 のユーザー意思（リスク許容度の宣言）**であり、無記録で消してはならない。したがって次の 3 フィールドを**すべて**記録した場合にのみ「運用移管」を選択できる。

| フィールド | 内容 |
|:-----------|:-----|
| **観測シグナル** | 発火したとき何が観測されるか（1 行） |
| **監視面** | **retro のみ**に固定する（新しいチェックリストファイルを作らない = 新 rules の胚を作らない） |
| **失効日** | この日まで沈黙が続けば懸念ごと消滅とみなす日付 |

**判定 3 条件（すべて充足する場合のみ運用移管を選択可）**:
...
**同時保有上限**: **5 件**。監視面が retro 1 回分の議題容量に制約されるため（LAM の retro 実績は Problem 7 / Try 8 程度）。上限は W1-M1-T6（PM 級一括承認）時点でユーザーが再確認する。

**リスク受容の承認**: 運用移管の判定はユーザーの PM 級承認（1 行）を要する（軸 1 の判断であるため）。

**再機構化の経路（新機構ゼロ）**: 移管済みリスクが発火した場合、それは `trust-model.md` §カウント単位が定める「**検出イベント**」1 カウントであり、閾値 2 で既存の draft rule 起票フローに自動合流する。**独自のエスカレーション機構は作らない**。失効日の意味は対称である — 窓内に発火すれば既存機構が拾い、沈黙のまま満了すれば懸念ごと消滅する（リスクが実在しなかった事後証明）。

#### 既存 3 概念との区別（意味論の SSOT / 2026-07-25 追加）
...
#### 立証責任の所在（Phase B / 2026-07-25 改訂）

**Phase B においては「残す」側に立証責任を課す。** 「発火ゼロ × 意図」セルの既定を**削減または運用移管**とし、**保全とする場合にのみ理由 1 行を必須**とする（削減側に理由は要求しない）。

**根拠**: ループを養うのは**規範ストックの総量**である。条項 1 本の真のコストは 4 費目に分かれる:

| # | 費目 | 内容 |
|:-:|:-----|:-----|
| (a) | **ロード時の認知負荷** | 重複・矛盾する指示の解決にモデルの注意が割かれる。**発火はタスクの真っ最中に起きる** |
| (b) | **保守コスト** | 条項は陳腐化し drift する（rule-001 / rule-002 の regex 連鎖が実測例） |
| (c) | **監査対象面** | 条項が存在する限り、次の監査は全件を嘗める |
| (d) | 起動時トークン | コンテキスト消費 |

> **注記（2026-07-25 / 根拠文の差し替え）**: 本節の初版は根拠を「指示は既定でコストである（Anthropic の Claude Code システムプロンプト 80% 削減実績）」に置いていたが、**この根拠は撤去する**。公式ドキュメントは「CLAUDE.md はシステムプロンプトの一部ではなく、その後に届くユーザーメッセージである」と明記しており、**80% はレイヤーが異なるため LAM に移植できない**（`docs/artifacts/m-1-baseline-w0.md` §W0-M1-T5 訂正 1）。立証責任の反転を支えるのは上表 (a)〜(c) であり、(d) ではない。
...
> **Phase A の副次出力**: Phase A で保全（veto）となった条項は、ロード条件を**「無条件ロード」固定**とする（§5.2.4）。追加の判定を要さない。

Phase B（残り精査 / Phase A で veto されなかった条項のみ）:

4. 帰属がモデル固有の事実（ADR-0011 決定 2 が定める「層への割当」「設計上の性質の説明」「時点記録」のいずれかに該当する記述）であれば **SSOT 退避**
5. **基質適合テスト**（2026-07-25 追加）: 「**Opus 5 は指示なしでこれをやるか**」を問う。**YES** の場合、当該条項の軸 4（実測発火）は Opus 4.x 以前の基質での観測であり**新基質での必要性の証拠として無効**であるため、軸 4 を「発火ゼロ」相当として扱い **削減または運用移管**へ送る
6. 5 で無効化されなかった条項について、軸 4（実測発火の有無）× 軸 2（意図か列挙か）の組み合わせで決定木の分岐を適用する。「発火ゼロ × 意図」セルの既定は**削減または運用移管**であり、**保全とする場合にのみ理由 1 行を必須**とする（§5.2 立証責任の所在）
7. **残存級（保全 / 圧縮）**となった条項にのみ、**ロード条件属性**を付与する（§5.2.4）。削減 / 運用移管 / 対象外の条項には付与しない

不変制約 4 対象に該当する条項は、Phase A の実施前に除外し 4 軸評価を経ない（FR-5 受け入れ条件 3）。

##### 基質適合テストの適用対象（手順 5 の補足）

本テストは**検証系足場**に対して特に効く。upstream 公式（[Prompting Claude Opus 5](https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5) / 2026-07-25 取得）は、旧世代向けの検証指示（"include a final verification step" / "use a subagent to verify" 型）が Opus 5 では **over-verification を引き起こし、品質を上げずトークンを浪費する**として削除を推奨し、**legacy harness scaffolding にも同じことが当てはまる**と明記している。

**W1 の判定候補**（機械的に削除してはならない / 個別判定すること）: `fable-l3-protocol.md` §4 自己監査 14 項目 / 同 §6.6 F4 全体検証 5 点 / `phase-rules.md` AUDITING チェックリスト群 / TDD 品質チェック R-1・R-4・R-5・R-6。

**gabriel の扱い（例外の可能性 / 必ず個別判定する）**: 公式が禁じているのは「**自己出力の検証**への subagent 使用」である。`gabriel` は ADR-0007 に基づき「同一モデルの別ペルソナ（MELCHIOR / BALTHASAR / CASPAR）は盲点が相関する」ことへの対処として導入された、**合議判断への脱相関コンテキストからの敵対 probe** であり、**対象クラスが異なる**。したがって基質適合テストを通過しうる。ただし通過は自動ではなく W1 で個別に判定する。参考実測（W0-M1-T3 項目 4）: `invoked=true` の 3 件すべてで verdict が `refuted`（母数 3 のため断定はしない）。

#### トリアージ表スキーマ

| 条項ID | 原文 | 出典（ファイル:節） | 軸1判定 | 軸2判定 | 軸3判定 | 軸4判定 | 基質適合 | 決定木の出力 | ロード条件 | 根拠1行 |
|:------|:-----|:-------------------|:-------|:-------|:-------|:-------|:--------|:-------------|:-----------|:--------|
| （例）`fable-l3-protocol.md#5.4-03` | （原文を転記） | `fable-l3-protocol.md` §5.4 | ユーザー意思 / モデル誤り予防 | 意図 / 列挙 | 不可逆 / 可逆 | 実測発火あり / 発火ゼロ | 通過 / 無効化 | 保全 / 圧縮 / 削減 / SSOT退避 / 運用移管 / 対象外 | 無条件 / 条件（glob）/ — | （1 行） |

「決定木の出力」列は「保全」「圧縮」「削減」「SSOT退避」「**運用移管**」「対象外」の **6 値の閉集合**とする。「対象外」は不変制約 4 対象に該当し 4 軸評価を経ていない条項に用いる。「ロード条件」列は残存級（保全 / 圧縮）にのみ値を持ち、それ以外は `—` とする。

### §5.2.4 ロード条件属性（配置属性 / 2026-07-25 追加）

#### 位置づけ: 判定軸と**直交する配置属性**であり、決定木の第 7 値ではない

決定木の 4 軸が判定しているのは条項の**内容**（誰の判断か / 形式 / 可逆性 / 根拠）であり、6 値は「この条項は存在に値するか、どの形で」への答えである。一方ロード条件は「**存在が確定した条項をいつ文脈に載せるか**」への答えであり、両者は独立に組み合わさる（「圧縮 × 条件ロード」「保全 × 無条件ロード」いずれも正当）。値として混ぜれば直積の分だけ値が爆発するか、表現できない組み合わせが生じる。

**決定的な理由は耐久性の非対称にある**: 決定木の出力（verdict）は条項内容への判断であり**永続する**。一方ロード方式は **upstream が握るプラットフォーム機構**であり、`paths:` の意味論変更・廃止は Anthropic の一存で起きうる。**揮発的なプラットフォーム機構に耐久的な統治判断を結合してはならない** — 属性であれば frontmatter の除去だけで巻き戻せるが、値にすると削減台帳と決定木の再判定が必要になる。

#### upstream 仕様（一次資料 / 2026-07-25 取得）

出典: [Memory — Claude Code Docs](https://code.claude.com/docs/en/memory)

- **`paths:` frontmatter を持たない rules は起動時に全文が無条件ロードされる**（`.claude/CLAUDE.md` と同じ優先度）
- **`paths:`（glob 配列）を付けると、Claude がマッチするファイルを「読んだとき」にのみロードされる**（"not on every tool use" と明記）
- LAM の実測（2026-07-25）: `.claude/rules/` 配下 18 ファイルのうち `paths:` を持つものは **0 件**

#### 判定基準（2 条件の連言 / これを満たすときのみ「条件ロード」可）

> **(A) トリガーがファイルに繋留されている ∧ (B) 当該行為に先行して対象ファイルの Read が保証される、または違反の事後回復が安価**

**(A) の判定** — LAM の規律はトリガーの繋留先が 3 種ある。`paths:` はファイル Read にしか反応しないため、**ファイル繋留以外は条件ロード不可**（glob の書きようがなく、「対象を見てから」という契機自体が存在しない）:

| 繋留先 | 例 | 条件ロード |
|:-------|:---|:----------:|
| **ファイル繋留** | 書式 / エンコーディング規約 / parser 保守 | **可** |
| **イベント・フェーズ繋留** | 承認ゲート / 完了宣言前ゲート / 体験シミュ発火点 | **不可** |
| **コマンド繋留** | 実行安全基準 | **不可** |

**(B) の判定** — Edit は事前 Read が構造的に保証されるため間に合う。一方**新規ファイルの Write は既存ファイルの Read を経ない**可能性があり、この経路の発火有無は**未検証**である。ここで (B) 後段が効く: **違反が事後回復可能（軸 3 で可逆側）であれば、届くのが遅れても損害は有界**である。逆に**不可逆ガードは (B) を絶対に満たせないため条件ロード不可**であり、これは **Phase A veto（軸 3）と完全に一致する**。すなわち**決定木が既に採取した軸 3 がそのままロード属性の判定に再利用でき、新規に問うのは (A) の 1 問のみ**である（W1 の判定 1 件あたり 1 問追加 / Task は増えない）。

未検証部分（Write 経路の発火）は `InstructionsLoaded` hook（どの指示ファイルがいつ・なぜロードされたかをログする）で後日確定できる。**確定を待たずに移行を開始してよい** — (B) 後段が未検証の仮定を無害化しているため。

#### ガード: 条件ロード化はトリアージの無効化装置にしない（MUST NOT）

**条件ロード化を理由として、削減判定を保全へ覆してはならない。** これを許すと「全条項に `paths:` を付けて何も削らない」という逃げ道が生まれ、規範ストックが丸ごと温存されてループが継続する。

条件ロードが消すコストは §5.2「立証責任の所在」の **(d) 起動時トークンのみ**であり、**(a) ロード時の認知負荷 / (b) 保守コスト / (c) 監査対象面 は無傷で残る**。さらに (a) には**悪化面**がある: 滅多に共ロードされない条項どうしの矛盾は、衝突機会が減る分だけ**発見が遅れる**（常時ロードの矛盾は日常的に露見するが、遅延ロードの矛盾は両 glob が同時マッチする稀な瞬間まで潜伏する）。

**条件ロード化が正当に動かせるのは「残す vs 消す」の境界ではなく、「残し方 2 形式（圧縮 vs 全文条件ロード）」の選択のみである。** 具体的には、列挙の詳細が「作業対象を開いた瞬間にこそ役立つ」型の条項（具体的なコードパターンを伴う規約等）については、意図 1 行への圧縮よりも「保全 + 条件ロード」が優ることがある。これは progressive disclosure の第 3 レベル（参照ファイルは必要時ロード）の条文版にあたる。
...
**確定**: `docs/artifacts/m-1-clause-ledger.md`。列は **8 種**（閉集合 / 2026-07-25 改訂）:

| 条項ID | 原文 | 出典（ファイル:節） | 判定軸（軸1〜4 + 基質適合） | 判定 | 移動先 / 監視条件 | ロード条件 | 判定日 |
|:------|:-----|:-------------------|:---------------------------|:-----|:------------------|:-----------|:-------|

**記録対象**: 本台帳には「**W2 で実ファイルに変更を加えた全件**」を記録する。判定が「保全」でも `paths:` frontmatter を付与した場合は記録対象に含める（変更を加えたため）。一方、判定が「保全」かつロード条件も無変更の条項は記録しない。
...
| 圧縮 | 意図 1 行に畳んだ |
| 削減 | 条文を削除した |
| SSOT 退避 | 別 SSOT へ移動した |
| **運用移管** | 機構での担保をやめ、運用（retro 監視）へ移した |
| **保全（ロード条件変更のみ）** | 条文は変えず `paths:` frontmatter のみ付与した |

> 決定木の出力は 6 値（§5.2）だが、台帳に載るのは実ファイルを変更した 5 パターンである。「保全（無変更）」と「対象外」は台帳に載らず、トリアージ表（`m-1-triage-table.md`）側で追跡する。
...
| **運用移管** | **`retro / シグナル: <1 行> / 失効: YYYY-MM-DD`**（§5.2「運用移管の必須要件」の 3 フィールドをこの 1 セルに記録する。**列を増やさない** = NFR-2 準拠） |
| 削減 | `—` |

**「ロード条件」列の書式**: `無条件` / `条件: <glob>` のいずれか。判定が「削減」「運用移管」の場合は `—`。

**NFR-2（帳簿単一原則）との整合**: 本台帳は記録表であり、成果物の合否を判定する帳簿ではない。成果物判定の帳簿は Green State 1 冊のみである（`fable-l3-protocol.md` §3 帳簿単一原則）。削減台帳への記載件数や記載の有無が Green State の判定条件（G1〜G5）に加算されることはない。**運用移管の 3 フィールドを独立列に展開せず 1 セルに畳むのも、列の増加が新しい帳簿の胚になることを避けるためである。**

**FR-7 の 3 列との関係**: FR-7 が要求する最小構成（原文 / 判定軸 / 移動先）を、上記 8 列（条項ID・出典・ロード条件・判定日を加えた拡張版）として満たす。W1 トリアージ表の判定件数と本台帳の記載件数は、判定日（W2 適用日）を基準に件数突合で確認する（FR-7 受け入れ条件 3）。**突合時は「保全（無変更）」「対象外」がトリアージ表にのみ存在することを織り込む**（件数の単純一致を条件にしない）。
...
4. **Red-5（削減台帳の配置パスと形式）**: §6.3 で確定（`docs/artifacts/m-1-clause-ledger.md` / **8 列** / 2026-07-25 に 7 列から改訂）
