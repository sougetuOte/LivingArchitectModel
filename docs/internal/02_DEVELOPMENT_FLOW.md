...
### Automated TDD Introspection (v4.0.0)

PostToolUse hook がテスト実行結果を自動監視し、以下を記録する:
- テスト失敗/成功パターンを `.claude/tdd-patterns.log` に蓄積（サマリ）
- 詳細パターンは `docs/artifacts/tdd-patterns/` にファイル単位で記録
- `src/` 配下のファイル変更を検知し、ドキュメント同期フラグを設定（詳細は `/ship` コマンド参照）

これにより、TDD サイクルの記録と仕様同期のトリガーが自動化される。
