...
- **PM級**: ask 応答で承認ダイアログを表示（対象パスは `.claude/rules/permission-levels.md`
  §ファイルパスベースの分類 が**正本** / ここに列挙を複製しない）

詳細は `.claude/rules/permission-levels.md` を参照。

> **列挙を参照に畳んだ理由（2026-09-04 / `retro-2026-09-04.md` Phase 2 検出）**: 旧記述は
> 「仕様書、ADR、ルールファイル、設定ファイル」と散文で列挙しており、**`CLAUDE.md`（2026-07-26 追加）と
> `docs/internal/`（2026-09-04 追加）の 2 件が欠落していた**。正本 1 枚 + 参照に畳むと
> この欠陥クラスが構造的に消える（2026-08-26 に `permission-levels.md` 内の複製 3 箇所へ適用したのと同型）。
