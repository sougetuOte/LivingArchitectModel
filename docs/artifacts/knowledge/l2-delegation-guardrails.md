...
1. **委譲は文脈の切断**。L1 が積んだセッション経緯は渡らず、subagent は「知らないことを知らない」ので質問も警告も出ない。R-2 のスキーマ誤りでは、参照すべき文書は**同日に自分たちが更新したファイル**だった（L1 に自明すぎて brief に書かれなかった）。**公式の明文あり**（2026-07-26 追記 / [Memory docs](https://code.claude.com/docs/en/memory)）: 「The main conversation's auto memory **isn't loaded into subagents**」— 親の auto memory すら渡らない設計
...
### 8.7 初回実測（2026-07-26 / §8 制定当日の 5 タスク）

§8.4 の判定（F0 の検証方法が実コマンドで書けたら委譲可）を実タスクに適用した結果。

| タスク | 判定 | 結果 |
|:--|:--|:--|
| R 系判定材料の収集 | 委譲（L2） | **成功**。29 tool_uses / 7.5 分 / 5 セクション完遂 |
| ADR-0001 の発生時期特定 | 委譲（L3） | **API 529 で異常終了**（環境起因）。残作業が git 1 コマンドだったため L1 が引き取り |
| `paths:` 発火条件の確定 | L1 直（PM 級境界の判断が残る） | 成功 |
| upstream 入力リスト作成 | L1 直（正解が外にない） | 成功 |
| memory tool の提供有無 | **委譲予定 → L1 直に変更** | 成功。照会 2 回で終わる仕事であり、Opus 5 ガイド #5「数回のツール呼び出しで済む仕事を委譲するな」に該当。**振り分け判定が実際に計画を覆した 1 例** |

**確認できたこと 3 点**:

1. **進捗の外形化が機能した**（§8.5 の訂正の初回実測）。boundaries に「N 件ごとに成果物へ進捗を追記せよ」を入れた結果、L1 は成果物ファイルの `<!-- progress: 5/5 -->` で完了を観測できた。M-1 P1 の「22 分 0 バイト」型の誤判定は起きなかった
2. **親検収が齟齬を捕捉した**。subagent は `test_10k_boundary` のヒットを 2 ファイルと報告したが、L1 の独立 grep では 3 ファイル。漏れた 1 件が**判定を左右する決定的資料**（当該条項を生んだ実インシデントの retro）だった。subagent は嘘をついておらず、指定された文字列で厳密に検索した結果、実体が `10k_boundary_exact` だったため原理的に届かなかった = **リテラル解釈が仕様どおり働いた取りこぼし**（MAST の FM-13 Incomplete Verification）
3. **§8.2 の予測どおりだった**。委譲が成功した唯一のタスクは「材料の実在を grep で突合できる」もので、判断を含む 3 件はいずれも L1 直が正しかった

**教訓**: 親検収は「成果物が正しいか」ではなく「**検索語が正しかったか**」を疑う。リテラル解釈のモデルに文字列を渡すと、その文字列の外は構造的に見えない。
