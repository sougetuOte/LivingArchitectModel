...
| **1b** | **Action 4b** = **skill の slash 形**（T3 側 85 箇所 / T1 チェーン 55 箇所）の名前空間化 | **次はここ** | **T1 の向きの扱いと同時に決める**。T1 は正本が `.claude/` 側で**順変換が存在しない**ため、T3 側だけ ns 化すると配布物の中で `/lam-harness:ship` と `/ship` が同居する。skill 名は `phase="building"` / `"command": "full-review"` のような**別名前空間の値**とも衝突するので一律変換は不可。下記 **#5（182 箇所）と同じ問題領域**であり、統合して設計する |
