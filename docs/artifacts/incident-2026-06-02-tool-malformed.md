...
## 追跡調査（2026-06-06・Opus 4.8 (1M) で再検証）

本日 Opus 4.8 (1M) で長時間・高負荷セッション（W-15/W-16 の TDD、full-review 並列サブエージェント、7コミット+push、~214k context まで）を実施したが **malformed 退行は無再現**（N=1）。一方で upstream の状況を `gh search issues anthropics/claude-code` で再確認した。

### upstream issue 状況（全件 OPEN＝未解決・直近更新 2026-06-05 まで）

| # | 要約 | 更新 |
|---|------|------|
| #63604 | Opus 4.8 が malformed tool_use 連発・応答全破棄（4.7 正常） | 06-04 |
| #64097 | Opus 4.8 malformed tool calls with parsing errors | 06-01 |
| #65453 | malformed タグが multi-step で silent parser failure | 06-04 |
| #64176 | 2.1.150-158 回帰・retry 耐性 | 05-31 |
| #64658 | 「could not be parsed」最近のデスクトップビルドでも再現 | 06-02 |
| #64990 | parsing failure + 無限 retry loop | 06-05 |
| #60033 | Write 大型 escape-dense（既存サブケース） | 06-01 |
| **#65247** | **大きな cache_read context + Extended Thinking で malformed + 高遅延** | 06-04 |

### 新知見と運用方針の更新

- **#65247 が最重要**: malformed が「大きな cache_read context + Extended Thinking」と相関するという報告。**高コンテキスト自体が malformed のトリガになりうる**ことを示唆。§4-4「長セッションほど確率が上がる」を upstream 報告が裏付けた形。
- 本セッションの高負荷無再現は「4.8 が短〜中コンテキストでは実用可」を示すが、**上記相関を踏まえると過信は禁物**。運用方針: (a) **200k 超で即 `/quick-save` → 新セッション**、(b) **超大型 context / Extended Thinking 多用は 4.7 (1M) を選ぶ**、(c) malformed 1回で即 4.7 フォールバックは従来どおり。
- CHANGELOG 未対応の理由は単純に **未修正だから**（2.1.160 時点で fix なし・§5 の結論不変）。
