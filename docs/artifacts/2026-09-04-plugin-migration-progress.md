...
| **11** | **第 1 段 E2E 合格（`bfe1aa1`）**。15 ステップ完走 / E1〜E6 全 pass / 陰性対照で E1・E4・E5・E6 が赤転。**欠陥 3 件を検出** —— (D-1) **bare `subagent_type` が解決しない**（配布 skills の agent 名言及は **97 箇所 / 12 名**）(D-2) `init/SKILL.md` の `${CLAUDE_PLUGIN_ROOT:?…}` ガードが必ず失敗する (D-3) E2 の証人の書き方が誤り（`.claude/rules` は 16）。**合格の限界**: E2/E3 は陰性対照で赤転しない / PM 級 Edit は headless で deny されるため PM キャッシュを証人にできず `permission.log` + `doc-sync-flag` へ差し替えた。詳細は `2026-09-05-magi-migration-sequence.md` §(D) |
