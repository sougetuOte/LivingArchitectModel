...
### B.4 実測: 条件ロード条項は compaction を生き残らない（2026-08-15 / 段 1）

本調査が文書上の記述として整理した「compaction 非生存層」を、**LAM 実環境で実測した**。文書の主張が実挙動と一致することを確認したため、以下は unverified ではない。

**手順**: ① `.claude/hooks/pre-compact.py` を Read し、`paths: .claude/**/*.py` を持つ `subprocess-encoding-convention.md` の条件ロードが system message として発火することを確認 → ② 圧縮前に答え合わせ表（規約内の具体値 7 問）を scratchpad に封入し、可視メッセージには書かない → ③ `/compact` を実行 → ④ 圧縮後の注入内容を直接観測。

**結果**:

| 対象 | 圧縮後 |
|:--|:--|
| 無条件常駐の `.claude/rules/` 13 件 + ルート `CLAUDE.md` | **生存** |
| **条件ロード条項（`subprocess-encoding-convention.md`）** | **脱落** |
| 引き金になった `pre-compact.py` 本体 | **脱落**（「直近 5 件」に入らなかった） |
| 直近に読んだファイル 5 件 | 生存 |

**裏取り**: 答え合わせ表が「直近 5 件」として復元されたため Q&A 経路は汚染された（下記）。代わりに、**答え合わせ表に載っていない規約内情報**（grep baseline の実コマンド / 正例・誤例のコード片 / 修正済 14 件のファイル名 / `_utf8_env()` の実装本体）を再現できないことで脱落を確認した。また残る 13 件に subprocess 文字コードを扱う条項はなく、**当該規範は条件ロード 1 本で運ばれており予備の配送経路を持たない**ことも確認した。

**副次の事実**:

- **PreCompact hook は正常動作**した（`SESSION_STATE.md` に `2026-08-15T12:37:55Z` を刻印）。`.claude/pre-compact-fired` の「2026-06-29 以降 47 日間発火ゼロ」はここで途切れる。
- **compaction が残すのは「直近に触れたもの」であり「統治しているもの」ではない**。本実験では答え合わせ表（作業ファイル）が残り、規範が消えた。
- 実験設計上の欠陥: 答え合わせ表を書いた直後に `/compact` を打ったため、それ自体が「直近 5 件」に入った。**再実験するなら答え合わせ表の作成後に無関係なファイルを 5 件以上読んでから圧縮する**。ただし直接観測で結論が出ているため再実験は不要と判断した。

**露出経路の評価**（頻度は運用依存 / 欠陥そのものは常在）: auto-compact = 1M 到達時のため 500K 運用では実質発火しない / **手動 `/compact` = 打てば必ず開く** / Resume from summary = CLI 限定でデスクトップ版の挙動は **unverified**。
