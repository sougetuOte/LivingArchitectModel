...
- **envelope 影響 (クレジット従量期 2 例目)**: 本 #14 は #13 に続くクレジット従量期 (2026-07-08 16:00 以降) 2 例目召喚。**brief-heavy loose 型としては初事例** (#13 は brief-heavy tight 型)。tool_uses 9 (mid) / duration ~6.6 分 は adversarial coverage の性質上 tool_uses 中規模帯に落ちる (#7 の 17 と #10 の 11 の間)。jsonl 実測は次日以降で確定するが、hga-summoning.md §envelope 定義 (実 $ envelope 月 $10-40) 内想定。
