...
| 関連仕様 | `docs/specs/approval-gate-v2/compatibility-matrix.md`（CC バージョン × `defaultMode` 配置可否 / v1.0.1・作成済） |
...
| v0.4 (Accepted・Phase B 完了) | 2026-07-02 | **Phase B 実装完了**（実装は 2026-06-30 commit `4a97fe2` / 本項は status 反映）。B-1: `docs/artifacts/incident-patterns.yaml` 新規（軸 4 C2 機械可読レジストリ）/ B-2: `pre-tool-use.py` 改修 + `_incident_patterns.py` 新規（軸 4 C2 動的 deny + C5 subagent 起動境界 / CC v2.1.178+ classifier との二重防御）/ B-3: `docs/specs/approval-gate-v2/compatibility-matrix.md` 新規（v1.0.0→v1.0.1）/ B-4: テスト 26 件（`test_incident_patterns.py` 18 + `test_subagent_boundary.py` 8 / 全 387 件緑）。これにより軸 4 の **C2・C5 が「実装予定」→「実装済（永続核）」に遷移**。残タスクは Phase C 末（軸 6 C4 重ね掛け撤去 3 条件の判断ゲート）+ Future considerations（C6 fallback / edit-on-approve）のみ。 |
