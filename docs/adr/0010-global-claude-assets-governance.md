...
## 追補 4（2026-09-07 / **パス参照と、配布集合の閉包** / ユーザー決定 + HGA #35 + MAGI 2 巡 + gabriel 2 回）

### 背景 —— 追補 3 は「名前」を拘束したが「パス」を拘束していなかった

追補 3 は不変条件を「**配布される側**に bare の**実行参照**が残っていないこと」へ一般化した。
これが拘束するのは **agent 名と skill 名**（I-4 の射程）であり、**パス参照は条文の字面の外にあった**。

Action 4c の実測（2026-09-07）は、その外側に 2 種の欠陥が在ることを示した。

**(1) 配布物のパス参照 91 件 / 192 箇所が利用者環境で解決しない。** 最大の参照元は
T1 の派生（利用者の `.claude/rules/` ・ `docs/internal/` になる層）であり、**そこでは
`${CLAUDE_PLUGIN_ROOT}` が展開されない** —— 上流の展開範囲は「Skill and agent content」に限られ、
T1 の出力は plugin component ではないためである
（`code.claude.com/docs/en/plugins-reference` §Environment variables / 2026-09-07 逐語取得）。

**(2) 配布物が、配布されない実体に依存していた。** 計器（`census_dangling.py`）が
コードスパンと Markdown リンクしか走査していなかったため見えていなかった ——

- 配布 skill の**フェンス内コマンド 13 箇所**が非配布のスクリプト・テストを名指ししており、
  `/lam-harness:quick-save` と `/lam-harness:full-review` は**利用者環境で実際に落ちる**
- 配布 hook `pre-tool-use.py` が**非配布の `docs/artifacts/incident-patterns.yaml`** を読み、
  **fail-open** する。すなわち **ADR-0008 Phase B-2a の動的 deny は、全利用者環境で沈黙したまま無効**だった

**(2) の原因は配布集合の中身ではなく、閉包を計算したエントリポイントが hook だけだったことである。**
...
> **配布される側の Markdown に残ってよいパス参照は、次のいずれかである。**
>
> - **(a)** 利用者環境で解決するもの
> - **(b)** `${CLAUDE_PLUGIN_ROOT}` を用いるもの（**プレースホルダが展開される層に限る**）
> - **(c)** 正典 URL
>
> **bare な repo 相対パスは配布される側に残さない。**
>
> **射程**: `.md` 以外（`.py` / `.sh` / `.json` / `.yaml`）と `templates/starter/**` は**射程外**である。
> 前者は読者が開発者であるため、後者は**利用者所有ファイル**であるため
> （starter は「以後はあなたの資産であり、plugin の更新では上書きされません」）。
>
> **射程内に解決しない参照が現れた場合、規則に例外を足さず<ins>本文を直す</ins>。**

**最後の 1 文が本追補の要である。** 追補 2・3 が繰り返し棄却してきた「人が 1 件ずつ承認する例外表」を
封じるための条項であり、Action 4b が codemod の diff レビューでブレース展開の破損を見つけたとき、
例外表に載せず**文章を直した**先例に従う。

**実際に射程内の残余が 2 件あった** —— `permission-levels.md` の
「判定は大文字小文字を区別しない」節が**証拠テキスト**として持つ `.claude/Rules/security-commands.md` と
`docs/Specs`。これらは本文修正で閉じる（gabriel が 2 巡にわたって指摘し、L1 が census の実出力で確認した）。

### 決定 2 —— K4 を「閉包の導出」へ拡張する

K4 は「配布集合 ⊆ 開発ロード集合」を定めるが、**配布集合そのものの決め方**を定めていなかった。

> **配布集合は<ins>エントリポイントからの到達閉包</ins>として導出する。**
>
> - **エントリポイント** = 配布 hooks ∪ **配布 Markdown のフェンス内コマンド**が名指しする実体
> - **到達関係** = import 閉包 ∪ **実行時データ依存**
>
> 配布されるコンポーネントが実行時に読むデータは、**配布されるか、
> その不在時の挙動が仕様として明示されていなければならない**（fail-open を黙認しない）。

**エントリポイントを「フェンス内コマンド」に限るのは構文的な線引きである。**
コードスパン内の散文参照（`` `.claude/tests/hooks/test_x.py` `` のような provenance の引用）は
**閉包の入力にしない** —— 読者が**実行する**ものと**読む**ものを構文で分け、
前者は閉包、後者は決定 1 の (c) 正典 URL で処理する。
「検証手段として名指ししているか」を文意で判定する規則は**採らない**（gabriel 2 巡目の指摘）。

### 変えないもの

- **統治不変条件 I-1 〜 I-6** / バックアップ B-1・B-3 / 移行順序 M-1・M-2
- **追補 2 決定 1 の向き** / 決定 3 / **追補 3 の表**（T1・T3 の正本と配布側の対応）
- **第 2 段のゲート**（HGA #31 4-a）
- **4a（R-A）・4b（R-S）の実装**。本追補が足すのは**パス参照の規則 R-P** であり、既存 2 規則は無変更

### 参照

- `docs/artifacts/2026-09-07-magi-action4c-path-references.md`（本決定の HGA #35 + MAGI 2 巡 + gabriel 2 回 / 規則 R-P の確定形と着手順）
- `docs/artifacts/hga-summon-log.md` §35（crux 5 点と裁定）
