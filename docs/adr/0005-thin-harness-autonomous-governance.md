...
本決定のスコープ外（詳細は [requirements §2](../specs/autonomous-mode/requirements.md) Non-Goals）:
- dynamic workflows / Agent Teams を core 依存として採用すること（future-candidates・実測後に判断）。
- 別プロジェクトへの fork / extract（Option C 却下）。
- 憲法本体（CLAUDE.md / docs/internal/）の思想変更。Hierarchy of Truth と Zero-Regression は維持。
- 自律ループの具体アルゴリズム・状態機械の設計（design で扱う）。
...
- **検証**: $100 Max で対象 spec を1本、自律統治モードで Green State まで到達させ、PM事項がキュー経由でのみ人間に上がる end-to-end が成立すること（requirements SC-1〜SC-7。SC-7 は自律エンジンが自身の統治ファイルを無断改変しないことの検証を含む）。
- **見直しトリガー**:
...
3. **機械的合意収束 ≠ 決定的**: adversarial verify は相関盲点で偽収束しうる。完了判定は決定的 Green State 接地（FR-4.1a）を唯一根拠とし、合意収束は非決定的観点（G3/G4）の補助に留める（**FR-9.3**）。RQ-4「checker は block」の根拠を補強。
...
## 注記: Accepted 後の整合修正（2026-05-30）

本 ADR は Accepted（2026-05-29）後、**決定内容を変えない範囲**で以下の整合・明確化を施した（autonomous-mode requirements のレビュー反映）。決定そのものは不変:
- 非スコープ（Non-Goals）節を明示（従来は requirements §2 のみに存在）。
- 検証方法の Success Criteria 範囲を SC-1〜SC-7 に更新（SC-7 追加に追従）。
- Reflection 追補の「FR-4.1」参照を、分割後の「FR-4.1a」に整合。
