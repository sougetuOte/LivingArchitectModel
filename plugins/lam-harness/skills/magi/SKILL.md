...
# /lam-harness:magi — 構造化意思決定（MAGI System v2）
...
**ユーザーが明示的に `/lam-harness:magi` を呼び出した場合は、条件に合致しなくても必ず実行する。**
...
議題を Atom（判断単位）に分解し、各 Atom が読む状態と書く状態を宣言する（依存 DAG はそこから導出する）。
...
4. **前提検証**: **分解表に「読む状態 / 書く状態」列があるか**。無いまま依存・並列・独立・同一を
   主張していれば、**それ自体を `severity=critical` とする**。列がある場合は、宣言された状態を
   **基質**（実ファイル・コード・設定）と照合し、**宣言が実際より小さくないか**を検査する
