...
- **termination 条件**: `rubric.md` の verify コマンドに基づき、`lam-harness:goal-driven-grader` が `overall: "pass"` を返すこと。grader 失敗を合格として扱わない（FR-2 MUST NOT）。
- **独立 verifier**: `lam-harness:goal-driven-grader`（独立した別コンテキストの Agent として起動・FR-2）。作業者（l3-executor）と同一コンテキストに混在せず、termination 判定の独立性を構造的に保証する。
