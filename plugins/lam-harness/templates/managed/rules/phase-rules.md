...
requirements / design / tasks の各成果物完成時、ユーザーへ承認要求を提出する**直前**に、承認判断をするユーザー本人を受け手として **60 秒実況 3-5 文** を行う (成果物毎 3 発火 / `docs/internal/08_EXECUTION_DISCIPLINE.md` §5.1)。
...
詳細ガード (劣化 3 形検出 = 同型構文化 / 成果物の説明文化 / 検証項目化 / 「詰まる瞬間」検出 3 回連続閾値 / 検知修正 1 文完結禁止) は `docs/internal/08_EXECUTION_DISCIPLINE.md` §5.4 参照。
...
Task 単位 (`tasks.md` の W7-B4-T9 相当) で F0-F4 を発動する。TDD 1 サイクル毎に F0 生成は儀式過剰のため Task 単位が正 (`docs/internal/08_EXECUTION_DISCIPLINE.md` §6.1)。
...
F0 は AoT 適用でも軽量モードでも常時実施。4 行の定義本体は `docs/internal/08_EXECUTION_DISCIPLINE.md` §6.2。
...
**試行上限** (F3 と連動 / `docs/internal/08_EXECUTION_DISCIPLINE.md` §6.7 準拠):
...
監査レポートを提出する**直前**に、レポートを読んで次の修正判断をする作業者 (自分の後続セッション or 別担当) を受け手として **60 秒実況 3-5 文** を行う (`docs/internal/08_EXECUTION_DISCIPLINE.md` §5.1)。
...
詳細ガード (劣化 3 形検出 = 同型構文化 / 成果物の説明文化 / 検証項目化 / 「詰まる瞬間」検出 3 回連続閾値) は `docs/internal/08_EXECUTION_DISCIPLINE.md` §5.4 参照。
