...
### gap が 0 になった —— 沈黙していた security 機構も塞いだ（2026-09-08 / Action 4c-1 完了）

**閉包 54 件 / 未配布 0 件。** 決定 A・C と小物 3 件を片付け、4c-1 を閉じた。

- **決定 A**: `subprocess-encoding-convention.md` の配布を止めた。**gap が 11 → 5 に落ちた** ——
  1 決定で参照 18 箇所と閉包の枝 6 件が同時に消える。`.claude/rules/` の正本は残る
- **決定 C**: `incident-patterns.yaml` を配り、hook のパス解決に
  **プロジェクト側 → plugin 同梱**のフォールバックを入れた。これが無い間、
  **ADR-0008 の動的 deny は全利用者環境で沈黙したまま無効**だった（緑のまま機能が無い形）。
  `source_md` 2 件は**マッチ時に利用者へ提示される**ので正典 URL 化した
- **小物 3 件**は手順書き換え。うち 2 件は prose ではなく **`[ -f X ] &&` ガード**にした ——
  追補 4 が要求する「不在時の挙動の明示」を、**実行可能な形**で書いたほうが強い

**計器にも同じ線を引いた**。`[ -f X ]` で守られた呼び出しは硬い依存に数えない ——
文意判定ではなく構文で切る（D4 がフェンスで「実行する / 読む」を切ったのと同じ手）。

**`--write` に自分の編集を巻き戻された。** 正本は plugin 側で、直していたのは派生側だった。
T3 の向き（ADR-0010 追補 2「正本は第 2 段の後に生き残る側」）を手で確かめた形になる。
...
実測が示した本体は別にあった —— **T1（`.claude/rules` `docs/internal` `.claude/scripts` →
`templates/managed/`）には生成器が無く、検査だけがあった**。つまり「同じ内容を 2 人が書き、
検査で一致を強制する」形が、**向きが逆であるという理由だけで温存されていた**
（HGA #33 が T3 について解体したのと同じ形）。

- **配布物の中で規約と実体が矛盾していた** —— `templates/starter/CLAUDE.md` は
  「**常に `/lam-harness:` を前置する**」と明文化し、`CHEATSHEET.md` はそれに従っている。
  一方で配布 skill 本体（67 行）と配布規範（51 行）は `/ship` `/retro` と書いていた。
  **規約を書いた層だけが規約を守っていた**
- **Ansible の FQCN 移行の最大の事故がそのまま当てはまった** —— あちらは「bare は修飾形の
  単なる短縮」という前提で autofix を作り、それが偽だった（bare `copy` は `ansible.legacy.copy` で
  **ローカル override を拾う**）。**我々も同じ構造を持ち、しかも LAM 自身が答えを書いていた** ——
  「名前空間を省くと、同名の personal / project skill があった場合にそちらが起動する」。
  **bare は短縮形ではない。** これは変換をやめる理由ではなく、変換が正しいことの根拠である
- **条文が T3 だけの前提で書かれていた**（gabriel が指摘）。ADR-0010 追補 2 決定 2 は
  「**正本側**に bare の実行参照が残っていないこと」と定めるが、これは「正本 = 配布される側」を
  暗黙に前提している。T1 は正本（`.claude/`）が配布されない側であり、文字どおり適用すると
  **LAM 自身が読む `.claude/rules/` を名前空間化することになり、LAM が壊れる**
- → **ADR-0010 追補 3**（PM 級 / ユーザー承認）で不変条件を
  「**配布される側**に bare の実行参照が残っていないこと」へ一般化した。
  条文自身の目的節（「緑のまま**配布物**が壊れる」）と整合する。**目的は変えず射程を正した**。
  T3 の判定は現行と一致し、4a の実装は無変更
- **規則 R-S**: skill 名は一般語なので **ハーネスの起動構文のみ**（slash 形 `/<name>` /
  `Skill(skill="<name>")`）。実測で `phase="building"` / `"command": "full-review"` /
  `"mode": "autonomous"` という**別名前空間の値**が存在し、一律 ns 化は壊す。
  **置換候補 125 箇所を全数レビューして誤爆 0 を確認した**（集計ではなく目視）
- **T1 生成器を新設**（`derive_managed_templates.py`）し、T1 の判定を
  「バイト恒等」→「**派生 == 導出(正本)**」へ改めた。**著者は `.claude/` 側を 1 つ書き、
  LAM 自身がそれを読み、利用者は導出物を読む**（入力 1・出力 2）。
  外部知見が一致していたのはこの形で、**「両方が正本」を採った例は 1 件も無かった**
- **diff レビューが意味の壊れた置換を 1 件捕まえた** ——
  `` `.claude/agents/{doc-writer,requirement-analyst,...}.md` `` という**ブレース展開のファイル列挙**が
  名前空間化されてパスが壊れていた。2 件目以降は直前が `,` で、`/` の lookbehind では捕まらない。
  規則の穴だったので **(P) 除外を「その語を含むトークンが `/` を含むか」で判定する**よう直した
  （区切りは空白とバッククォートのみ —— カンマを区切りにすると同じ穴が再発する）
- **テストが逆写像の定義域ズレを捕まえた** —— 往復恒等を素朴に `to_project_text` 一本で書くと、
  `verify_distributable_claims.py` のコメントが**意図的に持つ** `/lam-harness:init` を剥がして
  偽陽性で落ちる。`.py` は導出が恒等写像なのに逆写像だけが働いていた。
  **導出と逆写像は定義域が一致していなければならない**

T3 正本 16 ファイル / T1 派生 18 ファイルを変換。**`.claude/skills` `.claude/agents`
`.claude/rules` `docs/internal` の差分は 0 件** —— LAM 自身が読む正本は 1 バイトも変わっていない。
テストは **1400 → 1423 passed / 14 skipped**。設計と棄却案は
`docs/artifacts/2026-09-06-magi-action4b-skill-references.md`。
...
### 複製相の解体を実装した —— T3 を「バイト恒等」から「派生 == 導出(正本)」へ（2026-09-05）

ADR-0010 追補 2 の決定を機構に落とした（Action 3）。

- **生成器 `.claude/scripts/derive_project_copies.py` を新設**（`--check` / `--write`）。
  正本 `plugins/` から開発側 `.claude/` を、**名前空間 prefix の除去**によって導出する
- **T3 の判定を「バイト恒等」から「派生 == 導出(正本)」へ変更**。
  **検査と生成は同一の変換関数を使う**（別実装にすると両者がドリフトする）
- 名前空間も名前集合も**基質から導出**する（manifest の `name` / agents の stem + skills のディレクトリ名）。
  **維持リストを持たない**（機構 #7・#11 と同型）。manifest が読めなければ**恒等写像に落ちる**
  —— 推測で prefix を作らない
- **陰性対照 13 件**を追加。とくに `test_t3_detects_untransformed_dev_side` は
  **旧 T3（バイト恒等）なら緑のまま通っていた事象**を赤にする
- テスト **1351 → 1364 passed**

**導入初回の `--check` が、既存の欠陥と設計の穴を同時に検出した**。
`skills/release/SKILL.md` の「利用者環境の `/lam-harness:release` が存在しないスクリプトを呼んで落ちる」は
**実行指示ではなく、名前空間つき表記そのものを主題にした散文**であり、prefix を剥がすと主題が消える。
**HGA #33 の「prefix 除去は無損失」は文字列操作としては真だが、意味としては損失しうる**。
リテラルに依存しない言い換え（「利用者環境の本 skill が」）に直し、
**正本を書くときの約束**として生成器の docstring に実例つきで残した
（破っても沈黙はしない —— 生成器が対象ファイルを列挙し `git diff` に現れる）。
...
文書からではなく基質から数える方式を採り、**利用者環境（`/lam-harness:init` が敷く 44 ファイル + 空ディレクトリ 5
＋ plugin skills 15 / agents 12）をテンプレートの実在から再現**して、配布物 85 ファイルの全参照を突合した。

`verify_plugin_containment.py` も `verify_distributable_claims.py` も**緑のまま**、
**利用者環境で解決しない参照が 86 件 / 182 箇所**あった（**下界** —— 抽出はバッククォート span と
markdown リンクに限る）。`rule-001` 観測 #6 と同型（**緑のまま誤った状態を報告する**）であり、
記録にあった「60 件超」は managed 規範 → `docs/artifacts/` **のみ**を数えた部分集合だった。

- **新しく判明した構造**: **plugin 配布下では `.claude/skills/…` `.claude/agents/…` `.claude/hooks/…` という
  パス自己参照が原理的に壊れる**（実体は plugin キャッシュに置かれる / **37 箇所**）。
  project ファイルだった頃は解決していたため、**移行そのものが参照を壊した**類型であり、
  既存の切り分け基準（「LAM 固有の実体か」）では捕まらない
- **managed 規範が、利用者環境に存在しないテストを検証手段として処方していた**（13 箇所）
- **検査されていない領域**: T3（複製相）は同名トップレベルの**積集合**しか見ないため、
  **配布集合そのものが正しいか**は誰も検査していない。機構 #10 の走査対象は 4 glob のみで、
  **skills 27 + agents 12 + managed rules 14 は走査外**
- **是正 1（台帳）**: §C 機構 #11 の行が**実装より古かった**（`scripts` の追加と複製相 T3 が未反映）。
  実装に追随させ、**T3 の射程の限界**を補足に明記した
- **是正 2（確定版シナリオ）**: **E2 は「managed 36 + starter 8」と主張しながら証人は managed のみ**だったため
  starter 8 の証人を追加。**E6 は hooks 5 イベント中 1 本しか証人が無かった**ため
  `permission.log`（PreToolUse）を 2 本目に追加し、**残り 3 本が無証人である限界を明記**。
  **(C) に「合格が意味しないこと」を新設**（182 箇所は E1〜E6 を全て緑で通過する）
- **順序の決定（ユーザー / 2026-09-05）**: **参照の是正は E2E の後**。
  **37 箇所の正しい直し方は、E2E が plugin 配下の実体の見え方を実測してはじめて決まる**ため

詳細: `docs/artifacts/2026-09-05-distribution-scope-review.md`
