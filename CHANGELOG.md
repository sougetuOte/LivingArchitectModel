...
### 修正: PM 級パス列挙の複製を畳み、`CLAUDE.md` の欠落を閉じた（2026-08-26 / **PM 級** / §D 在庫 #5 消化）

**ドリフトの原因は「欠落」ではなく「複製」だった。** `permission-levels.md` には PM 級パスの列挙が **4 箇所**あり、ルート `CLAUDE.md` は**その 4 箇所すべてから同時に欠けていた**（実装 `_hook_utils.py` は 2026-07-26 から PM 判定していた / 約 4 週間未発見）。

正本を **§ファイルパスベースの分類 の 1 枚**に定め、他 3 箇所を参照に置換した。併せて等級定義の箇条書きと分類表に `CLAUDE.md` 行を追加（nested な `CLAUDE.md` は対象外 —— `/compact` 後に再注入されるのがルートのみで、常駐性の根拠がそこに限られるため）。**これでこの欠陥クラスは構造的に消える**（`rule-001` 恒久解 (c) / R3 機構 #7 と同型の手 = 一覧の保守をやめて基質から導く）。

**会計は射程内記述（無料）**。ただし 1 問（「消したら振る舞いが変わるか」）が最も際どかった事例でもある —— hook は PM ダイアログを出すが**「宣言」までは強制しない**ため、事前宣言義務への追加だけは素直に読むと課金側に落ちる。**決め手は実装のコメント**で、`^CLAUDE\.md$` は「条項追加が PM ダイアログ**も事前宣言義務も**経ない穴になっていた」という理由で追加・承認されていた。すなわち判断は 2026-07-26 に下されており、条文が追随していなかっただけである。**判定に迷ったら、機構がなぜ入ったかを読むこと。**
